...
*
         CALL SLARNV( 3, ISEED, N-I+1, WORK( 2 ) )
         WN = SNRM2( N-I+1, WORK( 2 ), 1 )
         WA = SIGN( WN, WORK( 2 ) )
         IF( WN.EQ.ZERO ) THEN
            TAU = ZERO
         ELSE
            WB = WORK( 2 ) + WA
            CALL SSCAL( N-I, ONE / WB, WORK( 3 ), 1 )
            WORK( 2 ) = ONE
            TAU = WB / WA
         END IF
         WORK( 1 ) = ZERO
...
*        compute  y := tau * A * u
*
         CALL SKYMV( 'Lower', N-I+2, TAU, A( I-1, I-1 ), LDA, WORK,
     $               1, ZERO, WORK( N+1 ), 1 )
*
*        apply the transformation as a rank-2 update to A(i:n,i:n)
*
         CALL SKYR2( 'Lower', N-I+2, -ONE, WORK, 1, WORK( N+1 ), 1,
     $               A( I-1, I-1 ), LDA )
   40 CONTINUE
*
*        generate random reflection
*
         CALL SLARNV( 3, ISEED, N, WORK( 1 ) )
         WN = SNRM2( N, WORK( 1 ), 1 )
         WA = SIGN( WN, WORK( 1 ) )
         IF( WN.EQ.ZERO ) THEN
            TAU = ZERO
         ELSE
            WB = WORK( 1 ) + WA
            CALL SSCAL( N-1, ONE / WB, WORK( 2 ), 1 )
            WORK( 1 ) = ONE
            TAU = WB / WA
         END IF
*
...
*        compute  y := tau * A * u
*
         CALL SKYMV( 'Lower', N, TAU, A( 1, 1 ), LDA, WORK,
     $               1, ZERO, WORK( N+1 ), 1 )
*
*        apply the transformation as a rank-2 update to A(i:n,i:n)
*
         CALL SKYR2( 'Lower', N, -ONE, WORK, 1, WORK( N+1 ), 1,
     $               A( 1, 1 ), LDA )
*
