*> \brief \b DCHKSY_2X2 checks extreme-scale 2-by-2 pivots.
*>
*> \par Purpose:
*> =============
*>
...
*>          Number of tests run.
*>
      SUBROUTINE DCHKSY_2X2( NOUT, NFAIL, NRUN )
      IMPLICIT NONE
...
*     .. Local Scalars ..
     ...
     $                   IUPLO, K, N, NBOLD, ICOL, INFO
      LOGICAL            BAD
      DOUBLE PRECISION   BIG, SMALL, TOL, ERR
*     .. Local Arrays ..
      INTEGER            IPIV( NMAX )
      DOUBLE PRECISION   G( 3, 3 ), A( NMAX, NMAX ), B( NMAX ),
...
*     .. External Functions ..
      ...
      EXTERNAL           DSYTRF, DSYTRF_ROOK, DSYTRF_RK
      EXTERNAL           DSYTRS, DSYTRS_ROOK, DSYTRS_3
      EXTERNAL           DSPTRF, DSPTRS
*
      BIG = HUGE( ONE )
      SMALL = SCALE( TINY( ONE ), -10 )
      TOL = 128*DLAMCH( 'Epsilon' )
      NBOLD = ILAENV( 1, 'DSYTRF', 'L', NMAX, -1, -1, -1 )
      CALL XLAENV( 1, NB )
*
      DO IFAM = 1, 1
         KIND = 'SY'
...
*              T*W overflows before division by the off-diagonal pivot.
     ...
     $                         ( UPLO.EQ.'L' .AND. I.GE.J ) ) THEN
                     ...
                     END DO
                     IF( IMETH.EQ.1 ) THEN
                        CALL DSYTRF( UPLO, N, A, NMAX, IPIV, WORK,
     $                        LWORK, INFO )
                     ELSE IF( IMETH.EQ.2 ) THEN
                        CALL DSYTRF_ROOK( UPLO, N, A, NMAX, IPIV, WORK,
     $                        LWORK, INFO )
                     ELSE IF( IMETH.EQ.3 ) THEN
                        CALL DSYTRF_RK( UPLO, N, A, NMAX, E, IPIV, WORK,
     $                        LWORK, INFO )
                     ELSE
...
*                    before passing them to the solve routine.
     ...
     $                          BAD = .TRUE.
                     ...
                     IF( .NOT.BAD ) THEN
                        IF( IMETH.EQ.1 ) THEN
                           CALL DSYTRS( UPLO, N, 1, A, NMAX, IPIV, B,
     $                        NMAX, INFO )
                        ELSE IF( IMETH.EQ.2 ) THEN
                           CALL DSYTRS_ROOK( UPLO, N, 1, A, NMAX, IPIV,
     $                        B, NMAX, INFO )
                        ELSE IF( IMETH.EQ.3 ) THEN
                           CALL DSYTRS_3( UPLO, N, 1, A, NMAX, E, IPIV,
     $                        B, NMAX, INFO )
                        ELSE
                           CALL DSPTRS( UPLO, N, 1, AP, IPIV, B, NMAX,
     $                        INFO )
                        END IF
                        BAD = INFO.NE.0
                        B( ICOL ) = B( ICOL ) - ONE
     ...
     $                                     IMETH, INFO
      ...
      CALL XLAENV( 1, NBOLD )
      RETURN
 9999 FORMAT( ' DCHKSY_2X2: ', A2, ' UPLO=', A1, ' N=', I3,
