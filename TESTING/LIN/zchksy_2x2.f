*> \brief \b ZCHKSY_2X2 checks extreme-scale 2-by-2 pivots.
*>
*> \par Purpose:
*> =============
*>
...
*>          Number of tests run.
*>
      SUBROUTINE ZCHKSY_2X2( NOUT, NFAIL, NRUN )
      IMPLICIT NONE
...
*     .. Local Scalars ..
     ...
     $                   IUPLO, K, N, NBOLD, ICOL, INFO
      LOGICAL            BAD
      DOUBLE PRECISION   BIG, SMALL, TOL, ERR, MAG
*     .. Local Arrays ..
      INTEGER            IPIV( NMAX )
      COMPLEX*16         G( 3, 3 ), A( NMAX, NMAX ), B( NMAX ),
...
*     .. External Functions ..
      ...
      INTEGER            ILAENV
      EXTERNAL           DLAMCH, ILAENV, XLAENV
      EXTERNAL           ZSYTRF, ZSYTRF_ROOK, ZSYTRF_RK
      EXTERNAL           ZSYTRS, ZSYTRS_ROOK, ZSYTRS_3
      EXTERNAL           ZHETRF, ZHETRF_ROOK, ZHETRF_RK
      EXTERNAL           ZHETRS, ZHETRS_ROOK, ZHETRS_3
      EXTERNAL           ZSPTRF, ZSPTRS, ZHPTRF
      EXTERNAL           ZHPTRS
*
      BIG = HUGE( ONE )
      SMALL = SCALE( TINY( ONE ), -10 )
      TOL = 128*DLAMCH( 'Epsilon' )
      NBOLD = ILAENV( 1, 'ZSYTRF', 'L', NMAX, -1, -1, -1 )
      CALL XLAENV( 1, NB )
*
      DO IFAM = 1, 2
         KIND = 'SY'
...
*              Also cover ordinary values and both reciprocal guards.
               MAG = BIG
               IF( IC.EQ.4 ) MAG = ONE
               IF( IC.EQ.5 ) MAG = SQRT( DLAMCH( 'S' ) ) / 16
               IF( IC.EQ.6 ) MAG = 16 / SQRT( DLAMCH( 'S' ) )
               G( 1, 2 ) = DCMPLX( MAG / 8, -MAG / 8 )
               G( 1, 3 ) = DCMPLX( -MAG / 2, -MAG / 2 )
               G( 2, 3 ) = G( 1, 3 )
               G( 3, 3 ) = G( 1, 2 )
               IF( IC.EQ.7 ) THEN
*                 Moderate pivot, large numerator: a pivot-only guard
*                 would form (-1.2-.4*i)*(.9+.34*i)*BIG and overflow.
                  G( 1, 2 ) = DCMPLX( .75D0, -.25D0 )
                  G( 1, 3 ) = DCMPLX( .4D0, .2D0 )
                  G( 2, 2 ) = DCMPLX( .3D0, -.2D0 )*BIG
                  G( 2, 3 ) = DCMPLX( -.7D0, -.3D0 )*BIG
                  G( 3, 3 ) = DCMPLX( -.2D0, -.3D0 )*BIG
               END IF
            END IF
            DO J = 1, 3
               DO I = 1, J - 1
...
*              Unitary diagonal scaling gives Hermitian imaginary pivots.
     ...
     $                         ( UPLO.EQ.'L' .AND. I.GE.J ) ) THEN
                     ...
                     IF( IFAM.EQ.1 ) THEN
                        IF( IMETH.EQ.1 ) THEN
                           CALL ZSYTRF( UPLO, N, A, NMAX, IPIV, WORK,
     $                        LWORK, INFO )
                        ELSE IF( IMETH.EQ.2 ) THEN
                           CALL ZSYTRF_ROOK( UPLO, N, A, NMAX, IPIV,
     $                        WORK, LWORK, INFO )
                        ELSE IF( IMETH.EQ.3 ) THEN
                           CALL ZSYTRF_RK( UPLO, N, A, NMAX, E, IPIV,
     $                        WORK, LWORK, INFO )
                        ELSE
                     ...
                     ELSE
                        IF( IMETH.EQ.1 ) THEN
                           CALL ZHETRF( UPLO, N, A, NMAX, IPIV, WORK,
     $                        LWORK, INFO )
                        ELSE IF( IMETH.EQ.2 ) THEN
                           CALL ZHETRF_ROOK( UPLO, N, A, NMAX, IPIV,
     $                        WORK, LWORK, INFO )
                        ELSE IF( IMETH.EQ.3 ) THEN
                           CALL ZHETRF_RK( UPLO, N, A, NMAX, E, IPIV,
     $                        WORK, LWORK, INFO )
                        ELSE
...
*                    before passing them to the solve routine.
                     ...
                     DO J = 1, N
                        DO I = 1, N
                           IF( .NOT.( ABS( DBLE( A( I, J ) ) ).LE.
     $                          BIG .AND. ABS( DIMAG( A( I, J ) ) ).LE.
     $                          BIG ) ) BAD = .TRUE.
                        END DO
                     END DO
                     DO I = 1, K
                        IF( .NOT.( ABS( DBLE( AP( I ) ) ).LE.
     $                          BIG .AND. ABS( DIMAG( AP( I ) ) ).LE.
     $                          BIG ) ) BAD = .TRUE.
                     END DO
                     DO I = 1, N
                        IF( .NOT.( ABS( DBLE( E( I ) ) ).LE.
     $                          BIG .AND. ABS( DIMAG( E( I ) ) ).LE.
     $                          BIG ) ) BAD = .TRUE.
                     END DO
...
*                       multiplier instead of a forward solve error.
     ...
     $                        DCMPLX( -.944D0, -.768D0 ) )
                     ...
                     IF( .NOT.BAD .AND. IC.NE.7 ) THEN
                        IF( IFAM.EQ.1 ) THEN
                           IF( IMETH.EQ.1 ) THEN
                              CALL ZSYTRS( UPLO, N, 1, A, NMAX, IPIV, B,
     $                        NMAX, INFO )
                           ELSE IF( IMETH.EQ.2 ) THEN
                              CALL ZSYTRS_ROOK( UPLO, N, 1, A, NMAX,
     $                        IPIV, B, NMAX, INFO )
                           ELSE IF( IMETH.EQ.3 ) THEN
                              CALL ZSYTRS_3( UPLO, N, 1, A, NMAX, E,
     $                        IPIV, B, NMAX, INFO )
                           ELSE
                              CALL ZSPTRS( UPLO, N, 1, AP, IPIV, B,
     $                        NMAX, INFO )
                           END IF
                        ELSE
                           IF( IMETH.EQ.1 ) THEN
                              CALL ZHETRS( UPLO, N, 1, A, NMAX, IPIV, B,
     $                        NMAX, INFO )
                           ELSE IF( IMETH.EQ.2 ) THEN
                              CALL ZHETRS_ROOK( UPLO, N, 1, A, NMAX,
     $                        IPIV, B, NMAX, INFO )
                           ELSE IF( IMETH.EQ.3 ) THEN
                              CALL ZHETRS_3( UPLO, N, 1, A, NMAX, E,
     $                        IPIV, B, NMAX, INFO )
                           ELSE
                              CALL ZHPTRS( UPLO, N, 1, AP, IPIV, B,
     $                        NMAX, INFO )
                           END IF
                        END IF
                        BAD = INFO.NE.0
                        ...
                        ERR = ZERO
                        DO I = 1, N
                           IF( .NOT.( ABS( DBLE( B( I ) ) ).LE.
     $                          BIG .AND. ABS( DIMAG( B( I ) ) ).LE.
     $                          BIG ) ) BAD = .TRUE.
                           ERR = MAX( ERR, ABS( B( I ) ) )
                        END DO
     ...
     $                                     IMETH, INFO
      ...
      CALL XLAENV( 1, NBOLD )
      RETURN
 9999 FORMAT( ' ZCHKSY_2X2: ', A2, ' UPLO=', A1, ' N=', I3,
