...
*  ===========
*
*      SUBROUTINE ZCHKCXX( DOTYPE, NM, MVAL, NN, NVAL,
...
*> \param[out] C
*> \verbatim
*>          C is COMPLEX*16 array, dimension (MMAX*NMAX)
*>          where MMAX is the maximum value of M in MVAL and NMAX is the
*>          maximum value of N in NVAL.
*> \endverbatim
*>
*> \param[out] COPYC
*> \verbatim
*>          COPYC is COMPLEX*16 array, dimension (MMAX*NMAX)
*>          where MMAX is the maximum value of M in MVAL and NMAX is the
*>          maximum value of N in NVAL.
*> \endverbatim
*>
*> \param[out] QRC
*> \verbatim
*>          QRC is COMPLEX*16 array, dimension (MMAX*NMAX)
*>          where MMAX is the maximum value of M in MVAL and NMAX is the
*>          maximum value of N in NVAL.
*> \endverbatim
*>
*> \param[out] COPYQRC
*> \verbatim
*>          COPYQRC is COMPLEX*16 array, dimension (MMAX*NMAX)
*>          where MMAX is the maximum value of M in MVAL and NMAX is the
*>          maximum value of N in NVAL.
*> \endverbatim
*>
*> \param[out] X
*> \verbatim
*>          X is COMPLEX*16 array, dimension (NMAX*NMAX)
*>          NMAX is the maximum value of N in NVAL.
*> \endverbatim
*>
*> \param[out] COPYX
*> \verbatim
*>          COPYX is COMPLEX*16 array, dimension (NMAX*NMAX)
*>          NMAX is the maximum value of N in NVAL.
*> \endverbatim
*>
*> \param[out] S
...
*>                a) ZLATMS: 3*max(M,N)
*>                b) ZQRT12: M*N + 2*min(M,N) + max(M,N)
*>                c) ZQPT01: M*N + N
*>                d) ZQRT11: M*M + M
*>
*>          (2) Optimal complex workspace dimension for ZGECXX.
*>              max( NMAX*NBMAX,                      \\ for ZGEQRF inside
*>                   NMAX*min(NBMAX_UNMQR,NBMAX)      \\ for ZUNMQR inside
*>                   + (NBMAX_UNMQR+1)*NBMAX_UNMQR ),
*>                   NBMAX*( NMAX + 1 ),              \\ for ZGEQP3RK inside
*>                   min(MMAX,NMAX) + NMAX*NBMAX )    \\ for ZGELS inside
*>              where NBMAX_UNMQR=64 is hardwired in ZUNMQR.
*>
...
*>              2*min(MMAX,NMAX)
*>              This is an upper bound for ZQRT12 routine 2*min(M,N).
*>
*>          (2) Optimal real workspace dimension for ZGECXX.
*>              2*NMAX
*>              This is an upper bound for ZGEQP3RK routine 2*N.
*>
...
*
            ...
            LDX = MAX( 1, N )
*
*           1) NOTE: for matrix generation routine ZLATMS, the workspace length
*           LWKTMS = 3*MAX( M, N ).  LWKTMS not used in the code.
*
*           2) Set workspace length for testing routines.
*             a) for ZQRT12, real LRWKTST = 2*MIN(M,N). LRWKTST not used in the code.
*                for ZQRT12, complex:
*
            LWKTST = MAX( 1, M*N + 2*MINMN + MAX( M, N ) )
*
*             b) for ZQPT01, complex:
*
            LWKTST = MAX( LWKTST, M*N + N )
*
*             c) for ZQRT11, complex:
*
...
*                 ZLATMS (which sets the array S) to generate zero matrix.
*
                  DO I = 1, MINMN
                     S( I ) = ZERO
                  END DO
...
*                 ZLATMS in S. We need to order singular values in S.
*
...
*
               CALL ZLASET( 'Full', M, N, CZERO, CZERO,
     $                      COPYC, LDC )
*
...
*
               CALL ZLASET( 'Full', MINMN, N, CZERO, CZERO,
     $                      COPYX, LDX )
*
*              Initialize a copy array for pivot IPIV for ZGECXX.
*
               DO I = 1, M
                  COPY_IPIV( I ) = 0
               END DO
*
*              Initialize a copy array for pivot JPIV for ZGECXX.
*
               DO J = 1, N
                  COPY_JPIV( J ) = 0
               END DO
*
*              Initialize a copy array COPY_DESEL_ROWS for ZGECXX.
*
               DO I = 1, M
                  COPY_DESEL_ROWS( I ) = 0
               END DO
*
*              Initialize a copy array COPY_SEL_DESEL_COLS for ZGECXX.
*
               DO J = 1, N
...
*
                  ...
                  CALL ZLACPY( 'All', M, N, COPYC, LDC, C, LDC )
                  CALL ZLACPY( 'All', M, N, COPYQRC, LDQRC, QRC, LDQRC )
                  CALL ZLACPY( 'All', MINMN, N, COPYX, LDX, X, LDX )
...
*                 Determine LWORK
*
*                 NBMAX_UNMQR is hardwired in ZUNMQR as NBMAX = 64.
*
                  NBMAX_UNMQR = 64
*
*                 a) For ZGEQRF inside ZGECXX, complex
*
                  LWORK = MAX( 1, N*NB )
*
*                 b) For ZUNMQR inside ZGECXX, complex
*
                  LWORK = MAX( LWORK,
     $             N*MIN(NBMAX_UNMQR,NB)+(NBMAX_UNMQR+1)*NBMAX_UNMQR )
*
*                 c1) For ZGEQP3RK inside ZGECXX, complex
*
                  LWORK = MAX( LWORK, NB*( N + 1 ) )
*
*                 c2) For ZGEQP3RK inside ZGECXX, real
*
                  LRWORK = MAX( 1, 2*N )
*
*                 d) For ZGELS inside ZGECXX, complex
*
...
*
                  LIWORK = MAX( 1, 2*N )
*
*                 Compute ZGECXX factorization of A.
*
                  SRNAMT = 'ZGECXX'
                  CALL ZGECXX( FACT, USESD, M, N,
     ...
     $                         IWORK, LIWORK, INFO )
*
*                 Check an error code from ZGECXX.
*
                  IF( INFO.LT.0 )
     $               CALL ALAERH( PATH, 'ZGECXX', INFO, 0, ' ',
     $                            M, N, NX, -1, NB, IMAT,
     $                            NFAIL, NERRS, NOUT )
*
*                 Compute test 1:
...
*
                  RESULT( 2 ) = ZQPT01( M, N, K, COPYA, A, LDA, TAU,
     $                                  JPIV, WORK, LWKTST )
*
*                 Compute test 3:
*
...
*
                  RESULT( 3 ) = ZQRT11( M, K, A, LDA, TAU, WORK,
     $                                  LWKTST )
*
                  NRUN = NRUN + 2
*
...
*                 The test returns the ratio:
*
*                 Returns 1.0D+38 if abs(R(j+1,j+1)) > abs(R(j,j)),
*                 j=1:K-1
*
...
*
     ...
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 ) 'ZGECXX', M, N,
