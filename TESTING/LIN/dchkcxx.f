...
*>                a) DLATMS: 3*max(M,N)
*>                b) DQRT12: max( M*N + 4*min(M,N) + max(M,N),
*>                                M*N + 2*min(M,N) + 4*N )
*>                c) DQPT01: M*N + N
*>                d) DQRT11: M*M + M
*>
...
*>                   2*NMAX + NBMAX*( NMAX + 1 ),     \\ for DGEQP3RK inside
...
*
            ...
            LDX = MAX( 1, N )
*
*           1) NOTE: for matrix generation routine DLATMS, the workspace length
...
*
                  NBMAX_ORMQR = 64
*
*                 a) For DGEQRF inside DGECXX
*
                  LWORK = MAX( 1, N*NB )
*
*                 b) For DORMQR inside DGECXX
*
                  LWORK = MAX( LWORK,
     $             N*MIN(NBMAX_ORMQR,NB)+(NBMAX_ORMQR+1)*NBMAX_ORMQR )
*
*                 c) For DGEQP3RK inside DGECXX
*
                  LWORK = MAX( LWORK, 2*N + NB*( N + 1 ) )
*
*                 d) For DGELS inside DGECXX
*
