...
*                       overflows.  CLATMS cannot generate such a
*                       matrix, because it scales its output by the
*                       requested norm.
*
                        IF( IMAT.EQ.9 ) THEN
                           SUBNRM = SLAMCH( 'Safe minimum' ) / 8
                           CALL CSSCAL( LDA*N, SUBNRM, A, 1 )
                        END IF
...
*
                           ANRMF = CLANGB( 'M', N, KL, KL+KU, AFAC,
     $                              LDAFAC, RWORK )
                           IF( SISNAN( ANRMF ) .OR.
     $                         ANRMF.GT.SLAMCH( 'Overflow' ) ) THEN
                              RESULT( 1 ) = ONE / SLAMCH( 'Epsilon' )
                           ELSE
                              RESULT( 1 ) = ZERO
                           END IF
                        ELSE
                           CALL CGBT01( M, N, KL, KU, A, LDA, AFAC,
     $                                 LDAFAC, IWORK, WORK,
     $                                 RESULT( 1 ) )
                        END IF
*
