...
*     .. External Subroutines ..
     ...
     $                   SLARHS, SLASET, SLATB4, SLATMS, SPOT07, SPOT09,
     $                   SKYSV, SSYSVX, SKYT01, SKYTRF, SKYTRI2, LSAME,
     $                   XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                  SRNAMT = 'SKYSVX'
                  CALL SKYSVX( FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA,
...
*
  ...
  130                CONTINUE
     ...
     $                  THEN
                  ...
                  END IF
*
*                 Check the error code from SKYSVX.
*
                  IF( INFO.NE.K ) THEN
                     CALL ALAERH( PATH, 'SKYSVX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from SKYSVX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'SKYSVX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
