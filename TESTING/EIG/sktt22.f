...
*  ===========
*
*       SUBROUTINE SKTT22( N, M, KBAND, AD, AE, SD, SE, U, LDU, WORK,
*                          LDWORK, RESULT )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> SKTT22  checks a set of M eigenvalues and eigenvectors,
*>
*>     A U = U S
*>
*> where A is skew-symmetric tridiagonal, the columns of U are orthogonal,
...
*> \verbatim
*>          N is INTEGER
*>          The size of the matrix.  If it is zero, SKTT22 does nothing.
...
*> \verbatim
*>          M is INTEGER
*>          The number of eigenpairs to check.  If it is zero, SKTT22
...
*>          is assumed to be skew-symmetric tridiagonal.  AE(1) is ignored,
...
*
      IF( N.GT.1 ) THEN
         ANORM = ABS( AE( 1 ) )
         DO 10 J = 2, N - 1
            ANORM = MAX( ANORM, ABS( AE( J ) )+
     $              ABS( AE( J-1 ) ) )
   10    CONTINUE
         ANORM = MAX( ANORM, ABS( AE( N-1 ) ) )
      ELSE
         ANORM = ZERO
      END IF
...
*
   ...
   40 CONTINUE
*
      WNORM = SLANKY( '1', 'L', M, WORK, M, WORK( 1, M+1 ) )
*
