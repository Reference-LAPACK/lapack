...
!
!     DSYDMD  for computation of the
!             Dynamic Mode Decomposition (DMD)
!     DSYDMDQ for computation of a
...
!............................................................
      ...
      REAL(KIND=WP), ALLOCATABLE, DIMENSION(:)   ::          &
                     DA, DL, DR, REIG, REIGA, REIGQ,    &
                     RES, RES1, RESEX, SINGVX,&
                     SINGVQX, WORK
...
!.............
      ...
      ALLOCATE( IWORK(2*M) )
      ALLOCATE(DR(N))
      CALL DLATMR( M, M, 'S', ISEED, 'S', DA, MODE, COND, &
      ...
      !! WRITE(*,'(A)', advance='no') & 
      !!'Compute and (if needed) store the eigenvalues &
      !!           &and eigenvectors of A (DSYEV) . ..'
      LWORK = MAX(1,3*M-1)
      ALLOCATE(WORK(LWORK))
      AC(1:M,1:M)  = A(1:M,1:M)  
      CALL DSYEV( 'V', 'U', M, AC, LDA, REIGA, WORK, LWORK, INFO ) ! LAPACK CALL 
      DEALLOCATE(WORK)
      
      TMP = ABS(REIGA(IDAMAX(M, REIGA, 1))) ! The spectral radius of A
      
      ! Scale A to have the desirable spectral radius.
      CALL DLASCL( 'G', 0, 0, TMP, ONE, M, M, A, M, INFO )
      CALL DLASCL( 'G', 0, 0, TMP, ONE, M, 1, REIGA, M, INFO )
...
!............................................................
      ...
      ! DSYDMD: Workspace query and workspace allocation
      CALL DSYDMD( SCALE, JOBZ, RESIDS, JOBREF, WHTSVD, WHTSYM, WHTEIG, M, &
      ...
      IF ( LSAME(JOBZ,'V')  ) THEN
          ! Check that Z = X*W, on return from DSYDMD
      ...
      IF ( LSAME(RESIDS, 'R') ) THEN
          ! Compare the residuals returned by DSYDMD with the
          ...
          ! pairs of eigencalues. (See the description of Z in DSYDMD,)
          i = 1
          DO WHILE ( i <= K )
                  ! have a real eigenvalue with real eigenvector
      ...
      !======================================================================
      !     Now test the DSYDMDQ
      !======================================================================
      IF ( TEST_QRDMD .AND. (K_TRAJ == 1) ) THEN
          F1 = F

          ! DSYDMDQ test: Workspace query and workspace allocation
          CALL DSYDMDQ( SCALE, JOBZ, RESIDS, WANTQ, WANTR, & 
               JOBREF, WHTSVD, WHTSYM, WHTEIG, M, N+1, F1, LDF, X, LDX, Y, & 
               LDY, NRNK, TOL, KQ, REIGQ, Z, LDZ,   &
          ...
          !..... DSYDMDQ check point
          IF ( LSAME(RESIDS, 'R') ) THEN
              ! Compare the residuals returned by DSYDMDQ with the
              ! explicitly computed residuals using the matrix A.
              ! Compute explicitly Y1 = A*Z
              CALL DGEMM( 'N', 'N', M, KQ, M, ONE, A, M, Z, M, ZERO, Y1, M )
              ! ... and then A*Z(:,i) - LAMBDA(i)*Z(:,i), using the real forms
              ! of the invariant subspaces that correspond to complex conjugate
              ! pairs of eigencalues. (See the description of Z in DSYDMDQ)
              i = 1
              DO WHILE ( i <= KQ )
                      ! have a real eigenvalue with real eigenvector
...
!............................................................
      ...
      IF ( NFAIL_TOTAL == 0 ) THEN
          WRITE(*,*) '>>>> DSYDMD :: ALL TESTS PASSED.'
      ELSE
          WRITE(*,*) NFAIL_TOTAL, 'FAILURES!'
          WRITE(*,*) '>>>>>>>>>>>>>> DSYDMD :: TESTS FAILED. CHECK THE IMPLEMENTATION.'
      END IF

      IF ( TEST_QRDMD ) THEN
      WRITE(*,*)
      WRITE(*,*) '>>>>>>>>>>>>>>>>>>>>>>>>>>'
      WRITE(*,*) ' Test summary for DSYDMDQ :'
      WRITE(*,*) '>>>>>>>>>>>>>>>>>>>>>>>>>>'
      WRITE(*,*)

      IF ( NFAIL_SVDIFF == 0 ) THEN
          WRITE(*,*) '>>>> DSYDMD and DSYDMDQ computed singular &
              &values test PASSED.'
      ELSE
          WRITE(*,*) 'DSYDMD and DSYDMDQ discrepancies in &
      ...
      IF ( NFAILQ_TOTAL == 0 ) THEN
          WRITE(*,*) '>>>>>>> DSYDMDQ :: ALL TESTS PASSED.'
      ELSE
         WRITE(*,*) NFAILQ_TOTAL, 'FAILURES!'
         WRITE(*,*) '>>>>>>> DSYDMDQ :: TESTS FAILED. CHECK THE IMPLEMENTATION.'
      END IF
