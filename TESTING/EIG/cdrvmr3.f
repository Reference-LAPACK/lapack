...
*  ===========
*
*       SUBROUTINE CDRVMR3( NSIZES, MM, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> CDRVMR3 checks the singular value decomposition (SVD) driver CGESVD,
*> CGESDD, CGESVJ, CGEJSV, CGESVDMR3, and CGESVDQ.
*>
...
*>
*> When CDRVMR3 is called, a number of matrix "sizes" (M's and N's)
...
*>        (Return 0 if true, 1/ULP if false.)
*>
*> Test for CGESVDMR3( 'V', 'V', 'A' )/CGESVDMR3( 'N', 'N', 'A' )
*>
...
*>          CDRVMR3 does nothing.  It must be at least zero.
*> \endverbatim
*>
*> \param[in] MM
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, CDRVMR3
...
*>          next call to CDRVMR3 to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*     .. External Subroutines ..
     ...
     $                   CGESVD, CGESVDQ, CGESVJ, CGEJSV, CGESVDMR3,
...
*
               ...
               SRNAMT = 'CGESVDMR3'
               CALL CGESVDMR3( 'V', 'V', 'A', M, N, A, LDA,
     $                       VL, VU, IL, IU, NS, SSAV, USAV, LDU,
     $                       VTSAV, LDVT, WORK, LWORK, RWORK,
     $                       IWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9995 )'GESVDMR3', IINFO, M, N,
     $               JTYPE, LSWORK, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
     ...
     $                      VTSAV, LDVT, WORK, RWORK, RESULT( 1 ) )
               IF( M.NE.0 .AND. N.NE.0 ) THEN
                  CALL CUNT01( 'Columns', M, MNMIN, USAV, LDU, WORK,
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) ) GO TO 160
                     ...
                     CALL CLACPY( 'F', M, N, ASAV, LDA, A, LDA )
                     SRNAMT = 'CGESVDMR3'
                     CALL CGESVDMR3( JOBU, JOBVT, 'A', M, N, A, LDA,
...
*
  ...
  180          CONTINUE
               ...
               CALL CLACPY( 'F', M, N, ASAV, LDA, A, LDA )
               SRNAMT = 'CGESVDMR3'
               CALL CGESVDMR3( 'V', 'V', 'I', M, N, A, LDA,
...
*
     ...
     $                    HALF*ABS( SSAV( NS )-SSAV( 1 ) ) )
               ...
               CALL CLACPY( 'F', M, N, ASAV, LDA, A, LDA )
               SRNAMT = 'CGESVDMR3'
               CALL CGESVDMR3( 'V', 'V', 'V', M, N, A, LDA,
...
*     Summary
*
      CALL ALASVM( 'CMR', NOUNIT, NERRS, NTESTT, 0 )
*
 9999 FORMAT( ' SVD -- ', A, ' Singular Value Decomposition Driver ',
     $      / ' CGESVDMR3 (MRRR dense driver)',
     $      / ' Matrix types (see CDRVMR3 for details):',
     ...
     $      / '  8 = | U**H A VT**H - diag(S) | / ( |A| max(M,N)',
     $      'ulp ), (V,V,I)',
     $      / '  9 = | I - U**H U | / ( M ulp ) ',
     $      / ' 10 = | I - VT VT**H | / ( N ulp ) ',
     $      / ' 11 = | U**H A VT**H - diag(S) | / ( |A| max(M,N)',
     $      'ulp ), (V,V,V)',
     $      / ' 12 = | I - U**H U | / ( M ulp ) ',
     $      / ' 13 = | I - VT VT**H | / ( N ulp ) ',
     $      / / )
 ...
 9996 FORMAT( ' CDRVMR3: ', A, ' returned INFO=', I6, '.', / 9X, 'M=',
     $      I6, ', N=', I6, ', JTYPE=', I6, ', ISEED=(', 3( I5, ',' ),
     $      I5, ')' )
 9995 FORMAT( ' CDRVMR3: ', A, ' returned INFO=', I6, '.', / 9X, 'M=',
