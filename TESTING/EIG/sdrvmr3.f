...
*  ===========
*
*       SUBROUTINE SDRVMR3( NSIZES, MM, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> SDRVMR3 checks the singular value decomposition (SVD) drivers
...
*>
*> When SDRVMR3 is called, a number of matrix "sizes" (M's and N's)
...
*>        (Return 0 if true, 1/ULP if false.)
*>
*> Test for SGESVDMR3( 'V', 'V', 'A' )/SGESVDMR3( 'N', 'N', 'A' )
*>
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, SDRVMR3
...
*>          SDRVMR3 to continue the same random number sequence.
*> \endverbatim
*>
*> \param[in] THRESH
*> \verbatim
...
*     .. External Subroutines ..
     ...
     $                   SGESVDQ, SGESVDMR3, SGESVJ, SLACPY, SLASET,
...
*
               ...
               CALL SGESVDMR3( 'V', 'V', 'A', M, N, A, LDA,
...
*
     ...
     $                      VTSAV, LDVT, WORK, RESULT( 1 ) )
               IF( M.NE.0 .AND. N.NE.0 ) THEN
                  CALL SORT01( 'Columns', M, MNMIN, USAV, LDU, WORK,
     $                         LWORK,
     $                         RESULT( 2 ) )
                  CALL SORT01( 'Rows', MNMIN, N, VTSAV, LDVT, WORK,
     $                         LWORK,
     $                         RESULT( 3 ) )
               END IF
               RESULT( 4 ) = ZERO
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) )GO TO 170
                     ...
                     RANGE = CJOBR( 1 )
                     CALL SLACPY( 'F', M, N, ASAV, LDA, A, LDA )
                     CALL SGESVDMR3( JOBU, JOBVT, RANGE, M, N, A, LDA,
...
*
  ...
  200          CONTINUE
               ...
               END IF
               CALL SLACPY( 'F', M, N, ASAV, LDA, A, LDA )
               CALL SGESVDMR3( 'V', 'V', 'I', M, N, A, LDA,
...
*
     ...
     $                      RESULT( 10 ) )
*
*              Do tests 11--13: SGESVDMR3( 'V', 'V', 'V' )
*
     ...
     $                    HALF*ABS( SSAV( NS )-SSAV( 1 ) ) )
               ...
               END IF
               CALL SLACPY( 'F', M, N, ASAV, LDA, A, LDA )
               CALL SGESVDMR3( 'V', 'V', 'V', M, N, A, LDA,
...
*
 9999 FORMAT( ' SVD -- ', A, ' Singular Value Decomposition Driver ',
     $      / ' SGESVDMR3 (MRRR dense driver)',
     $      / ' Matrix types (see SDRVMR3 for details):',
 ...
 9996 FORMAT( ' SDRVMR3: ', A, ' returned INFO=', I6, '.', / 9X, 'M=',
     $      I6, ', N=', I6, ', JTYPE=', I6, ', ISEED=(', 3( I5, ',' ),
     $      I5, ')' )
 9995 FORMAT( ' SDRVMR3: ', A, ' returned INFO=', I6, '.', / 9X, 'M=',
