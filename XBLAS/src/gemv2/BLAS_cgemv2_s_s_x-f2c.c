...
extern void FC_FUNC_(blas_cgemv2_s_s_x, BLAS_CGEMV2_S_S_X)
 
  (int *trans, int *m, int *n, const void *alpha, const float *a, int *lda,
   const float *head_x, const float *tail_x, int *incx, const void *beta,
   void *y, int *incy, int *prec) {
  BLAS_cgemv2_s_s_x(blas_colmajor, (enum blas_trans_type) *trans, *m, *n,
