...
void do_test_zhemv_z_c
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemv_z_c";
		...
		for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
		  ...
		  for (incy_val = INCY_START; incy_val <= INCY_END;
			...
			continue;
		      ...
		      BLAS_zhemv_z_c_testgen(norm, order_type,
...
void do_test_zhemv_c_z
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_c_z";
		...
		for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
		  ...
		  for (incy_val = INCY_START; incy_val <= INCY_END;
			...
			continue;
		      ...
		      /* finally we are here to generate the test case */
		      BLAS_zhemv_c_z_testgen(norm, order_type,
					     uplo_type, n, randomize_val,
					     &alpha, alpha_flag, &beta,
					     beta_flag, a, lda, x, incx, y,
					     incy, seed, head_r_true,
...
void do_test_zhemv_c_c
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_c_c";
		...
		for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
		  ...
		  for (incy_val = INCY_START; incy_val <= INCY_END;
			...
			continue;
		      ...
		      /* finally we are here to generate the test case */
		      BLAS_zhemv_c_c_testgen(norm, order_type,
					     uplo_type, n, randomize_val,
					     &alpha, alpha_flag, &beta,
					     beta_flag, a, lda, x, incx, y,
					     incy, seed, head_r_true,
...
void do_test_chemv_c_s
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_chemv_c_s";
		...
		for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
		  ...
		  for (incy_val = INCY_START; incy_val <= INCY_END;
			...
			continue;
		      ...
		      /* finally we are here to generate the test case */
		      BLAS_chemv_c_s_testgen(norm, order_type,
					     uplo_type, n, randomize_val,
					     &alpha, alpha_flag, &beta,
					     beta_flag, a, lda, x, incx, y,
					     incy, seed, head_r_true,
...
void do_test_zhemv_z_d
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_z_d";
		...
		for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
		  ...
		  for (incy_val = INCY_START; incy_val <= INCY_END;
			...
			continue;
		      ...
		      /* finally we are here to generate the test case */
		      BLAS_zhemv_z_d_testgen(norm, order_type,
					     uplo_type, n, randomize_val,
					     &alpha, alpha_flag, &beta,
					     beta_flag, a, lda, x, incx, y,
					     incy, seed, head_r_true,
	  ...
	  for (test_no = 0; test_no < ntests; test_no++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (incx_val = INCX_START; incx_val <= INCX_END;
			...
			/* finally we are here to generate the test case */
			BLAS_chemv_testgen(norm, order_type,
					   uplo_type, n, randomize_val,
					   &alpha, alpha_flag, &beta,
					   beta_flag, a, lda, x, incx, y,
					   incy, seed, head_r_true,
...
void do_test_zhemv_x
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_x";
	  ...
	  for (test_no = 0; test_no < ntests; test_no++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (incx_val = INCX_START; incx_val <= INCX_END;
			...
			/* finally we are here to generate the test case */
			BLAS_zhemv_testgen(norm, order_type,
					   uplo_type, n, randomize_val,
					   &alpha, alpha_flag, &beta,
					   beta_flag, a, lda, x, incx, y,
					   incy, seed, head_r_true,
...
void do_test_zhemv_z_c_x
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_z_c_x";
...
void do_test_zhemv_c_z_x
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_c_z_x";
...
void do_test_zhemv_c_c_x
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_c_c_x";
...
void do_test_chemv_c_s_x
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_chemv_c_s_x";
...
void do_test_zhemv_z_d_x
  (int n,
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests) {

  /* Function name */
  const char fname[] = "BLAS_zhemv_z_d_x";
...
int main(int argc, char **argv)
{
  ...
  if (argc != 6) {
    printf("Usage:\n");
    printf("do_test_hemv <nsizes> <ntests> <thresh> <debug> <test_prob>\n");
  ...
  if (nsizes < 0 || ntests < 0 || debug < 0 || debug > 3)
    BLAS_error("Testing hemv", 0, 0, NULL);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_z_c(n, ntests, &seed, thresh, debug,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_c_z(n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_c_c(n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemv_c_s(n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_z_d(n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemv_x(n, ntests, &seed, thresh, debug,
		    test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		    &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_x(n, ntests, &seed, thresh, debug,
		    test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		    &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_z_c_x(n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_c_z_x(n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_c_c_x(n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemv_c_s_x(n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemv_z_d_x(n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
