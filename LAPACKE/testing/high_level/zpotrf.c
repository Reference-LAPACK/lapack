...
#define LAPACKE_ZPOTRF_ALLOC_TEST(layout_index, countdown, name, expected)     \
    do {                                                                       \
        ...
        lapacke_test_check(name, lapacke_test_layout_names[layout_index],      \
                           LAPACKE_zpotrf(layout, 'U', N, a, LD), expected);   \
    } while (0)
...
LAPACKE_TEST(zpotrf)
{
    ...
    for (size_t l = 0; l < 2; l++) {
        ...
        LAPACKE_TEST_ZNAN_SWEEP("zpotrf a uplo=U", l, N, N, a, LD,
                                lapacke_test_region_upper, -4,
                                (lapacke_test_zfill_spd(layout, N, a, LD)),
                                LAPACKE_zpotrf(layout, 'U', N, a, LD));

        LAPACKE_TEST_ZNAN_SWEEP("zpotrf a uplo=L", l, N, N, a, LD,
                                lapacke_test_region_lower, -4,
                                (lapacke_test_zfill_spd(layout, N, a, LD)),
                                LAPACKE_zpotrf(layout, 'L', N, a, LD));
        ...
        lapacke_test_check(
            ...
            LAPACKE_zpotrf(layout, 'U', N, a, LD) >= 0 ? 0 : -999, 0);
    ...
    LAPACKE_ZPOTRF_ALLOC_TEST(1, 1, "zpotrf recovers after alloc failure", 0);
    lapacke_test_check_alloc_count("zpotrf allocation count");
