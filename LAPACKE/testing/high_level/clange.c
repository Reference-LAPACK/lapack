...
/* Refill the inputs, schedule the malloc failure, call clange. */
...
LAPACKE_TEST(clange)
{
    ...
    lapacke_test_check("clange alloc failure (work) returns 0.0", "col-major",
                       res == 0 ? 0 : -999, 0);
    ...
    lapacke_test_schedule_malloc_failure(0);
    res = API_SUFFIX(LAPACKE_clange)(LAPACK_ROW_MAJOR, 'I', M, N, a, LD);
    lapacke_test_check("clange alloc failure (work) returns 0.0", "row-major",
                       res == 0 ? 0 : -999, 0);
    LAPACKE_CLANGE_ALLOC_TEST(1, 1, 'I', "clange allocation count", 0);
    lapacke_test_check_alloc_count("clange row-major allocation count");
