...
#define LAPACKE_CPOTRF_ALLOC_TEST(layout_index, countdown, name, expected)     \
    do {                                                                       \
        ...
        lapacke_test_check(name, lapacke_test_layout_names[layout_index],      \
                           LAPACKE_cpotrf(layout, 'U', N, a, LD), expected);   \
    } while (0)
...
LAPACKE_TEST(cpotrf)
{
    ...
    for (size_t l = 0; l < 2; l++) {
        ...
        LAPACKE_TEST_CNAN_SWEEP("cpotrf a uplo=U", l, N, N, a, LD,
                                lapacke_test_region_upper, -4,
                                (lapacke_test_cfill_spd(layout, N, a, LD)),
                                LAPACKE_cpotrf(layout, 'U', N, a, LD));

        LAPACKE_TEST_CNAN_SWEEP("cpotrf a uplo=L", l, N, N, a, LD,
                                lapacke_test_region_lower, -4,
                                (lapacke_test_cfill_spd(layout, N, a, LD)),
                                LAPACKE_cpotrf(layout, 'L', N, a, LD));
        ...
        lapacke_test_check(
            ...
            LAPACKE_cpotrf(layout, 'U', N, a, LD) >= 0 ? 0 : -999, 0);
