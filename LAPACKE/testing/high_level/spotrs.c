...
/* Refill the inputs, schedule the malloc failure, call spotrs. */
...
LAPACKE_TEST(spotrs)
{
    ...
    for (size_t l = 0; l < 2; l++) {
        ...
        LAPACKE_TEST_SNAN_SWEEP(
            "spotrs b uplo=U", l, N, NRHS, b, LD, lapacke_test_region_full, -7,
            (lapacke_test_sfill_spd(layout, N, a, LD),
             lapacke_test_sfill_rhs(layout, N, NRHS, b, LD)),
            API_SUFFIX(LAPACKE_spotrs)(layout, 'U', N, NRHS, a, LD, b, LD));
        ...
        LAPACKE_TEST_SNAN_SWEEP(
            "spotrs b uplo=L", l, N, NRHS, b, LD, lapacke_test_region_full, -7,
            (lapacke_test_sfill_spd(layout, N, a, LD),
             lapacke_test_sfill_rhs(layout, N, NRHS, b, LD)),
            API_SUFFIX(LAPACKE_spotrs)(layout, 'L', N, NRHS, a, LD, b, LD));
    ...
    LAPACKE_SPOTRS_ALLOC_TEST(1, 0, "spotrs transpose alloc failure (a_t)",
                              LAPACK_TRANSPOSE_MEMORY_ERROR);
    LAPACKE_SPOTRS_ALLOC_TEST(1, 1, "spotrs transpose alloc failure (b_t)",
                              LAPACK_TRANSPOSE_MEMORY_ERROR);
