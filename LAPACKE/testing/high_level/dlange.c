...
/* Refill the inputs, schedule the malloc failure, call dlange. */
...
LAPACKE_TEST(dlange)
{
    ...
    lapacke_test_check("dlange alloc failure (work) returns 0.0", "col-major",
                       res == 0 ? 0 : -999, 0);
    LAPACKE_DLANGE_ALLOC_TEST(0, 1, 'I', "dlange allocation count", 0);
    lapacke_test_check_alloc_count("dlange col-major allocation count");

    lapacke_test_dfill(LAPACK_ROW_MAJOR, M, N, a, LD);
    lapacke_test_schedule_malloc_failure(0);
    res = API_SUFFIX(LAPACKE_dlange)(LAPACK_ROW_MAJOR, 'I', M, N, a, LD);
    lapacke_test_check("dlange alloc failure (work) returns 0.0", "row-major",
                       res == 0 ? 0 : -999, 0);
    LAPACKE_DLANGE_ALLOC_TEST(1, 1, 'I', "dlange allocation count", 0);
    lapacke_test_check_alloc_count("dlange row-major allocation count");
