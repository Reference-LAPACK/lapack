...
/* Refill the inputs, schedule the malloc failure, call sgeqrf. */
...
LAPACKE_TEST(sgeqrf)
{
    ...
    LAPACKE_SGEQRF_ALLOC_TEST(0, 0, "sgeqrf work alloc failure (work)",
                              LAPACK_WORK_MEMORY_ERROR);
    LAPACKE_SGEQRF_ALLOC_TEST(0, 1, "sgeqrf allocation count", 0);
    lapacke_test_check_alloc_count("sgeqrf col-major allocation count");

    /* row-major: the workspaces, then the transposed copies */
    LAPACKE_SGEQRF_ALLOC_TEST(1, 0, "sgeqrf work alloc failure (work)",
                              LAPACK_WORK_MEMORY_ERROR);
    LAPACKE_SGEQRF_ALLOC_TEST(1, 1, "sgeqrf transpose alloc failure (a_t)",
                              LAPACK_TRANSPOSE_MEMORY_ERROR);
    LAPACKE_SGEQRF_ALLOC_TEST(1, 2, "sgeqrf allocation count", 0);
    lapacke_test_check_alloc_count("sgeqrf row-major allocation count");
