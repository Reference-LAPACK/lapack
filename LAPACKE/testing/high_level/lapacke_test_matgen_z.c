 ...
 */
static void lapacke_test_zfill_latms(int layout, lapack_int rows,
                                     lapack_int cols, lapack_complex_double *a,
                                     ...
                                     lapack_int ku, const lapack_int seed[4])
{
    ...
    iseed[2] = seed[2];
    iseed[3] = seed[3];
    LAPACK_zlatms(&rows, &cols, &dist, iseed, &sym, d, &mode, &cond, &dmax, &kl,
    ...
    for (p = 0; p < len; p++) {
        if (lapacke_test_map_position(layout, rows, cols, ld, p, &i, &j)) {
            ...
            a[p] = lapack_make_complex_double(
                0.25, 0.0); /* padding, never referenced */
        }
    }
}
                        ...
                        lapack_complex_double *a, lapack_int ld)
{
    ...
    lapacke_test_zfill_latms(layout, rows, cols, a, ld, 'N', rows - 1, cols - 1,
                             seed);
}

/**
 * \brief Fill a with a random Hermitian positive definite n-by-n matrix, double
 * precision complex.
 *
                            ...
                            lapack_int ld)
{
    ...
    lapacke_test_zfill_latms(layout, n, n, a, ld, 'P', n - 1, n - 1, seed);
}

/**
 * \brief Fill a with a random Hermitian indefinite n-by-n matrix, double
 * precision complex.
 *
                            ...
                            lapack_int ld)
{
    ...
    lapacke_test_zfill_latms(layout, n, n, a, ld, 'H', n - 1, n - 1, seed);
}
                            ...
                            lapack_complex_double *a, lapack_int ld)
{
    ...
    lapacke_test_zfill_latms(layout, n, n, a, ld, 'N', upper ? 0 : n - 1,
                             upper ? n - 1 : 0, seed);
}

/**
 * \brief Fill b with a random right-hand side, double precision complex.
 *
 * Same generator as lapacke_test_zfill with a different seed, so right-hand
                            ...
                            lapack_complex_double *b, lapack_int ld)
{
    ...
    lapacke_test_zfill_latms(layout, rows, cols, b, ld, 'N', rows - 1, cols - 1,
                             seed);
}
                            ...
                            lapack_complex_double *a, lapack_int ld)
{
    ...
    for (size_t p = 0; p < len; p++) {
        a[p] = lapack_make_complex_double(lapacke_create_nan(),
                                          lapacke_create_nan());
    }
}

/**
 * \brief A quiet NaN, double precision complex.
 *
 * \return A quiet NaN in both parts.
 */
lapack_complex_double lapacke_test_znan(void)
{
    return lapack_make_complex_double(lapacke_create_nan(),
                                      lapacke_create_nan());
}
                           ...
                           lapack_complex_double *ab, lapack_int ld)
{
    ...
    lapacke_test_zfill_latms(LAPACK_COL_MAJOR, m, n, tmp, LAPACKE_TEST_LD, 'N',
                             kl, ku, seed);
    for (p = 0; p < len; p++) {
        if (lapacke_test_map_position(layout, rows, n, ld, p, &i, &j) &&
            ...
            ab[p] = lapack_make_complex_double(0.25, 0.0); /* never referenced */
        }
    }
}
...
static void lapacke_test_zscatter_band(int layout, char uplo, lapack_int n,
                                       lapack_int kd, const lapack_complex_double *tmp,
                                       lapack_complex_double *ab, lapack_int ld)
{
    const int upper = (uplo == 'U' || uplo == 'u');
 ...
 * matrix in the uplo band storage, double precision complex.
 *
 * \param[in]  layout LAPACK_COL_MAJOR or LAPACK_ROW_MAJOR.
                           ...
                           lapack_complex_double *ab, lapack_int ld)
{
    ...
    lapack_complex_double tmp[LAPACKE_TEST_LD * LAPACKE_TEST_LD];
    lapacke_test_zfill_latms(LAPACK_COL_MAJOR, n, n, tmp, LAPACKE_TEST_LD, 'P',
                             kd, kd, seed);
    lapacke_test_zscatter_band(layout, uplo, n, kd, tmp, ab, ld);
}

/**
 * \brief Fill ab with a random nonsingular triangular n-by-n band matrix in
 * the uplo band storage, double precision complex.
 *
 * \param[in]  layout LAPACK_COL_MAJOR or LAPACK_ROW_MAJOR.
 * \param[in]  uplo   'U' or 'L'.
 * \param[in]  n      Matrix order (at most LAPACKE_TEST_LD).
                           ...
                           lapack_complex_double *ab, lapack_int ld)
{
    ...
    const int upper = (uplo == 'U' || uplo == 'u');
    lapack_complex_double tmp[LAPACKE_TEST_LD * LAPACKE_TEST_LD];
    lapacke_test_zfill_latms(LAPACK_COL_MAJOR, n, n, tmp, LAPACKE_TEST_LD, 'N',
...
static void lapacke_test_zpack(int layout, char uplo, lapack_int n,
                               const lapack_complex_double *tmp, lapack_complex_double *ap)
{
 ...
 * definite n-by-n matrix in packed storage, double precision complex.
 *
 * The same matrix as lapacke_test_zfill_spd, packed.
 *
 * \param[in]  layout LAPACK_COL_MAJOR or LAPACK_ROW_MAJOR.
 * \param[in]  uplo   'U' or 'L'.
 * \param[in]  n      Matrix order (at most LAPACKE_TEST_LD).
 ...
 */
void lapacke_test_zfill_pp(int layout, char uplo, lapack_int n, lapack_complex_double *ap)
{
    lapack_complex_double tmp[LAPACKE_TEST_LD * LAPACKE_TEST_LD];
    lapacke_test_zfill_spd(LAPACK_COL_MAJOR, n, tmp, LAPACKE_TEST_LD);
    lapacke_test_zpack(layout, uplo, n, tmp, ap);
}

/**
 * \brief Fill ap with the uplo triangle of a random Hermitian indefinite
 * n-by-n matrix in packed storage, double precision complex.
 *
 * The same matrix as lapacke_test_zfill_sym, packed.
 *
 * \param[in]  layout LAPACK_COL_MAJOR or LAPACK_ROW_MAJOR.
 * \param[in]  uplo   'U' or 'L'.
 * \param[in]  n      Matrix order (at most LAPACKE_TEST_LD).
 * \param[out] ap     Buffer of n * (n + 1) / 2 elements.
 */
void lapacke_test_zfill_sp(int layout, char uplo, lapack_int n, lapack_complex_double *ap)
{
    lapack_complex_double tmp[LAPACKE_TEST_LD * LAPACKE_TEST_LD];
    lapacke_test_zfill_sym(LAPACK_COL_MAJOR, n, tmp, LAPACKE_TEST_LD);
    lapacke_test_zpack(layout, uplo, n, tmp, ap);
}

/**
 * \brief Fill ap with a random nonsingular uplo triangular n-by-n matrix in
 * packed storage, double precision complex.
 *
 * The same matrix as lapacke_test_zfill_tri, packed.
 *
 * \param[in]  layout LAPACK_COL_MAJOR or LAPACK_ROW_MAJOR.
 * \param[in]  uplo   'U' or 'L'.
 * \param[in]  n      Matrix order (at most LAPACKE_TEST_LD).
 * \param[out] ap     Buffer of n * (n + 1) / 2 elements.
 */
void lapacke_test_zfill_tp(int layout, char uplo, lapack_int n, lapack_complex_double *ap)
{
    lapack_complex_double tmp[LAPACKE_TEST_LD * LAPACKE_TEST_LD];
    lapacke_test_zfill_tri(LAPACK_COL_MAJOR, uplo, n, tmp, LAPACKE_TEST_LD);
    lapacke_test_zpack(layout, uplo, n, tmp, ap);
}
...
void lapacke_test_zfill_vec(lapack_int len, lapack_complex_double *v)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        const double im = (double)((i * 5 + 2) % 7) / 7.0 - 0.5;
        v[i] = lapack_make_complex_double(re, im);
    }
}

/**
 * \brief Fill v with len deterministic values in [2, 3), double precision complex.
 *
...
void lapacke_test_zfill_pos(lapack_int len, lapack_complex_double *v)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        v[i] = lapack_make_complex_double(re, 0.0);
    }
}
