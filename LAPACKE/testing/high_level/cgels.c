...
/* Refill the inputs, schedule the malloc failure, call cgels. */
...
LAPACKE_TEST(cgels)
{
    ...
    LAPACKE_CGELS_ALLOC_TEST(0, 0, "cgels work alloc failure (work)",
                             LAPACK_WORK_MEMORY_ERROR);
    LAPACKE_CGELS_ALLOC_TEST(0, 1, "cgels allocation count", 0);
    lapacke_test_check_alloc_count("cgels col-major allocation count");

    /* row-major: the workspaces, then the transposed copies */
    LAPACKE_CGELS_ALLOC_TEST(1, 0, "cgels work alloc failure (work)",
                             LAPACK_WORK_MEMORY_ERROR);
    LAPACKE_CGELS_ALLOC_TEST(1, 1, "cgels transpose alloc failure (a_t)",
                             LAPACK_TRANSPOSE_MEMORY_ERROR);
    LAPACKE_CGELS_ALLOC_TEST(1, 2, "cgels transpose alloc failure (b_t)",
                             LAPACK_TRANSPOSE_MEMORY_ERROR);
    LAPACKE_CGELS_ALLOC_TEST(1, 3, "cgels allocation count", 0);
    lapacke_test_check_alloc_count("cgels row-major allocation count");
