...
/* Refill the inputs, schedule the malloc failure, call dpotrs. */
...
LAPACKE_TEST(dpotrs)
{
    ...
    for (size_t l = 0; l < 2; l++) {
        ...
        LAPACKE_TEST_DNAN_SWEEP(
            "dpotrs b uplo=U", l, N, NRHS, b, LD, lapacke_test_region_full, -7,
            (lapacke_test_dfill_spd(layout, N, a, LD),
             lapacke_test_dfill_rhs(layout, N, NRHS, b, LD)),
            API_SUFFIX(LAPACKE_dpotrs)(layout, 'U', N, NRHS, a, LD, b, LD));
        ...
        LAPACKE_TEST_DNAN_SWEEP(
            "dpotrs b uplo=L", l, N, NRHS, b, LD, lapacke_test_region_full, -7,
            (lapacke_test_dfill_spd(layout, N, a, LD),
             lapacke_test_dfill_rhs(layout, N, NRHS, b, LD)),
            API_SUFFIX(LAPACKE_dpotrs)(layout, 'L', N, NRHS, a, LD, b, LD));
    ...
    LAPACKE_DPOTRS_ALLOC_TEST(1, 0, "dpotrs transpose alloc failure (a_t)",
                              LAPACK_TRANSPOSE_MEMORY_ERROR);
    LAPACKE_DPOTRS_ALLOC_TEST(1, 1, "dpotrs transpose alloc failure (b_t)",
                              LAPACK_TRANSPOSE_MEMORY_ERROR);
