...
/* Refill the inputs, schedule the malloc failure, call dgeqrf. */
#define LAPACKE_DGEQRF_ALLOC_TEST(layout_index, countdown, name, expected)     \
    do {                                                                       \
        ...
        lapacke_test_check(                                                    \
            ...
            API_SUFFIX(LAPACKE_dgeqrf)(layout, M, N, a, LD, tau), expected);   \
    } while (0)
...
LAPACKE_TEST(dgeqrf)
{
    ...
    for (size_t l = 0; l < 2; l++) {
        ...
        LAPACKE_TEST_DNAN_SWEEP(
            "dgeqrf a", l, M, N, a, LD, lapacke_test_region_full, -4,
            (lapacke_test_dfill(layout, M, N, a, LD),
             lapacke_test_dfill_vec(LD * LD, tau)),
            API_SUFFIX(LAPACKE_dgeqrf)(layout, M, N, a, LD, tau));
        ...
        lapacke_test_check(
            ...
            API_SUFFIX(LAPACKE_dgeqrf)(layout, M, N, a, LD, tau) < 0, 0);
        LAPACKE_set_nancheck(1);
    }

    /* column-major: the high-level workspaces */
    LAPACKE_DGEQRF_ALLOC_TEST(0, 0, "dgeqrf work alloc failure (work)",
    ...
    LAPACKE_DGEQRF_ALLOC_TEST(1, 0, "dgeqrf work alloc failure (work)",
                              LAPACK_WORK_MEMORY_ERROR);
    LAPACKE_DGEQRF_ALLOC_TEST(1, 1, "dgeqrf transpose alloc failure (a_t)",
