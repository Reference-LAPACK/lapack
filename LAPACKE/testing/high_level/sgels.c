...
#define LAPACKE_SGELS_ALLOC_TEST(layout_index, countdown, name, expected)      \
    do {                                                                       \
        ...
        lapacke_test_check(                                                    \
            ...
            LAPACKE_sgels(layout, 'N', M, N, NRHS, a, LD, b, LD), expected);   \
    } while (0)
...
LAPACKE_TEST(sgels)
{
    ...
    for (size_t l = 0; l < 2; l++) {
        ...
        LAPACKE_TEST_SNAN_SWEEP(
            ...
            (lapacke_test_sfill(layout, M, N, a, LD),
             lapacke_test_sfill_rhs(layout, M, NRHS, b, LD)),
            LAPACKE_sgels(layout, 'N', M, N, NRHS, a, LD, b, LD));
        ...
        lapacke_test_check(
            "sgels NaN with nancheck off", lapacke_test_layout_names[l],
            LAPACKE_sgels(layout, 'N', M, N, NRHS, a, LD, b, LD) >= 0 ? 0
                                                                      : -999,
            0);
    ...
    LAPACKE_SGELS_ALLOC_TEST(1, 3, "sgels recovers after alloc failure", 0);
    lapacke_test_check_alloc_count("sgels allocation count");
