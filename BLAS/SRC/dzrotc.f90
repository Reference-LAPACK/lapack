!> \brief \b DZROTC applies a chain of rotation sequences to a matrix.
!
!  =========== DOCUMENTATION ===========
!
...
!  ===========
!
!   subroutine dzrotc(side, dir, startup, shutdown, m, n, k,&
!                    A, lda, C, ldc, S, lds)
!    .. Scalar Arguments ..
...
!> \verbatim
!>
!> DZROTC applies a chain of k rotation sequences of length n to a matrix A.
!>
...
!> [ A(i+1,j) ]   [ -S(i,j)  C(i,j) ] [ A(i+1,j) ]
!> If side = 'R', rotation G(i,j) is applied to columns j and j+1 of A.
!> [ A(i,j)   A(i,j+1)   ] = [ A(i,j)   A(i,j+1)   ] [  C(i,j) -S(i,j) ]
...
!> \param[in,out] C
!> \verbatim
!>          C is DOUBLE PRECISION array, dimension (ldc,k)
...
!    .. Array Arguments ..
    double complex, intent(inout) :: A(lda,*)
