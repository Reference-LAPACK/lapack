...
*  are read using the format ( A16, L2 ). An annotated example of a data
...
*  cblas_sskewsymv  T PUT F FOR NO TEST. SAME COLUMNS.
...
*  cblas_sskewsyr2  T PUT F FOR NO TEST. SAME COLUMNS.
*
*     See:
*
...
*           Test SGEMV, 01, and SGBMV, 02.
  140       IF (CORDER) THEN
     ...
     $                  X, XX, XS, Y, YY, YS, YT, G, 1 )
            END IF
            GO TO 200
*           Test SSYMV, 03, SSBMV, 04, and SSPMV, 05, and SSKEWSYMV, 17.
  150       IF (CORDER) THEN
...
*           Test SSYR, 13, and SSPR, 14.
  180       IF (CORDER) THEN
     ...
     $                  YT, G, Z, 1 )
            END IF
            GO TO 200
*           Test SSYR2, 15, and SSPR2, 16, and SSKEWSYR2, 18.
  190       IF (CORDER) THEN
...
10000 FORMAT( ' COLUMN-MAJOR DATA LAYOUT IS TESTED' )
 ...
 9987 FORMAT( ' AMEND DATA FILE OR INCREASE ARRAY SIZES IN PROGRAM',
     $      /' ******* TESTS ABANDONED *******' )
 9986 FORMAT( ' SUBPROGRAM NAME ',A16, ' NOT RECOGNIZED', /' ******* T',
     $      'ESTS ABANDONED *******' )
 9985 FORMAT( ' ERROR IN SMVCH -  IN-LINE DOT PRODUCTS ARE BEING EVALU',
     ...
     $      , /' ******* TESTS ABANDONED *******' )
 9984 FORMAT(A16, L2 )
 9983 FORMAT( 1X,A16, ' WAS NOT TESTED' )
...
*
  140 CONTINUE
     ...
     $   WRITE( NOUT, FMT = 9979 )SNAME, NTESTS, NFAILS
      IF( IORDER.EQ.1 )
     $   WRITE( NOUT, FMT = 9978 )SNAME, NTESTS, NFAILS
      RETURN
*
10003 FORMAT( ' ', A16,' COMPLETED THE ROW-MAJOR    COMPUTATIONAL ',
     $ 'TESTS (', I6, ' CALLS)', /' ******* BUT WITH MAXIMUM TEST ',
     $ 'RATIO ', F8.2, ' - SUSPECT *******' )
10002 FORMAT( ' ', A16,' COMPLETED THE COLUMN-MAJOR COMPUTATIONAL ',
     $ 'TESTS (', I6, ' CALLS)', /' ******* BUT WITH MAXIMUM TEST ',
     $ 'RATIO ', F8.2, ' - SUSPECT *******' )
10001 FORMAT( ' ', A16,' PASSED THE ROW-MAJOR    COMPUTATIONAL TESTS',
     $ ' (', I6, ' CALL', 'S)' )
10000 FORMAT( ' ', A16,' PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS',
     $ ' (', I6, ' CALL', 'S)' )
 9998 FORMAT( ' ******* FATAL ERROR - PARAMETER NUMBER ', I2, ' WAS CH',
     $      'ANGED INCORRECTLY *******' )
 9997 FORMAT( ' ',A16, ' COMPLETED THE COMPUTATIONAL TESTS (', I6, ' C',
     $      'ALLS)', /' ******* BUT WITH MAXIMUM TEST RATIO', F8.2,
     $      ' - SUSPECT *******' )
 9996 FORMAT( ' ******* ',A16, ' FAILED ON CALL NUMBER:' )
 9995 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', 4( I3, ',' ), F4.1,
     $      ', A,', I3, ',',/ 10x, 'X,', I2, ',', F4.1, ', Y,',
     $      I2, ') .' )
 9994 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', 2( I3, ',' ), F4.1,
     $      ', A,', I3, ', X,', I2, ',', F4.1, ', Y,', I2,
     $      ') .' )
 9993 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',
     $      '******' )
 9979 FORMAT( ' ', A16,' COLUMN-MAJOR COMPUTATIONAL TESTS:', I9,
     $      ' RUN,', I9, ' FAILED' )
 9978 FORMAT( ' ', A16,' ROW-MAJOR    COMPUTATIONAL TESTS:', I9,
     $      ' RUN,', I9, ' FAILED' )
*
*     End of SCHK1.
*
...
*  =====================================================================
     ...
     $                  XS, Y, YY, YS, YT, G, IORDER )
      IMPLICIT NONE
*
*  Tests SSYMV, SSBMV and SSPMV, SSKEWSYMV.
*
*  Auxiliary routine for test program for Level 2 Blas.
*
*  -- Written on 10-August-1987.
...
*
     ...
     $                                   LDA, XX, INCX, BETA, YY, INCY )
                           ELSE IF( SKEWFULL )THEN
                              IF( TRACE )
     $                           WRITE( NTRA, FMT = 9993 )NC, SNAME,
     $                           CUPLO, N, ALPHA, LDA, INCX, BETA, INCY
                              IF( REWI )
     $                           REWIND NTRA
                              CALL CSSKEWSYMV( IORDER, UPLO, N, ALPHA,
...
10000 FORMAT( ' ', A16,' PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS',
 ...
 9997 FORMAT( ' ',A16, ' COMPLETED THE COMPUTATIONAL TESTS (', I6, ' C',
     ...
     $      ' - SUSPECT *******' )
 9996 FORMAT( ' ******* ',A16, ' FAILED ON CALL NUMBER:' )
 9995 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', I3, ',', F4.1, ', AP',
     $      ', X,', I2, ',', F4.1, ', Y,', I2, ') .' )
 9994 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', 2( I3, ',' ), F4.1,
     $      ', A,', I3, ', X,', I2, ',', F4.1, ', Y,', I2,
     $      ') .' )
 9993 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', I3, ',', F4.1, ', A,',
     $      I3, ', X,', I2, ',', F4.1, ', Y,', I2, ') .' )
...
10000 FORMAT( ' ', A16,' PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS',
 ...
 9997 FORMAT( ' ',A16, ' COMPLETED THE COMPUTATIONAL TESTS (', I6, ' C',
     ...
     $      ' - SUSPECT *******' )
 9996 FORMAT( ' ******* ',A16, ' FAILED ON CALL NUMBER:' )
 9995 FORMAT( 1X, I6, ': ',A16, '(', 3( A14,',' ),/ 10x, I3, ', AP, ',
     $      'X,', I2, ') .' )
 9994 FORMAT( 1X, I6, ': ',A16, '(', 3( A14,',' ),/ 10x, 2( I3, ',' ),
     $      ' A,', I3, ', X,', I2, ') .' )
 9993 FORMAT( 1X, I6, ': ',A16, '(', 3( A14,',' ),/ 10x, I3, ', A,',
     $      I3, ', X,', I2, ') .' )
 9992 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',
     $      '******' )
 9979 FORMAT( ' ', A16,' COLUMN-MAJOR COMPUTATIONAL TESTS:', I9,
...
10000 FORMAT( ' ', A16,' PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS',
 ...
 9994 FORMAT( 1X, I6, ': ',A16, '(', 2( I3, ',' ), F4.1, ', X,', I2,
     $      ', Y,', I2, ', A,', I3, ')                  .' )
 9993 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',
     $      '******' )
 9979 FORMAT( ' ', A16,' COLUMN-MAJOR COMPUTATIONAL TESTS:', I9,
...
10000 FORMAT( ' ', A16,' PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS',
 ...
 9996 FORMAT( ' ******* ',A16, ' FAILED ON CALL NUMBER:' )
 9995 FORMAT( '      THESE ARE THE RESULTS FOR COLUMN ', I3 )
 9994 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', I3, ',', F4.1, ', X,',
     $      I2, ', AP) .' )
 9993 FORMAT( 1X, I6, ': ',A16, '(', A14, ',', I3, ',', F4.1, ', X,',
     $      I2, ', A,', I3, ') .' )
 9992 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',
     $      '******' )
 9979 FORMAT( ' ', A16,' COLUMN-MAJOR COMPUTATIONAL TESTS:', I9,
...
*  =====================================================================
     ...
     $                  Z, IORDER )
      IMPLICIT NONE
*
*  Tests SSYR2 and SSPR2, SSKEWSYR2.
*
*  Auxiliary routine for test program for Level 2 Blas.
*
*  -- Written on 10-August-1987.
...
*     .. External Functions ..
      ...
      EXTERNAL           LSE, LSERES
*     .. External Subroutines ..
      EXTERNAL           SMAKE, SMVCH, CSSPR2, CSSYR2, CSSKEWSYR2
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
*     .. Scalars in Common ..
      INTEGER            INFOT, NOUTC
...
*
     ...
     $                     ALPHA, INCX, INCY, LDA
                        IF( REWI )
     $                     REWIND NTRA
                        CALL CSSKEWSYR2( IORDER, UPLO, N, ALPHA, XX,
     $                              INCX, YY, INCY, AA, LDA )
                     ELSE IF( PACKED )THEN
                        IF( TRACE )
     $                     WRITE( NTRA, FMT = 9994 )NC, SNAME, CUPLO, N,
   ...
   80                      CONTINUE
                        ...
                        DO 90 J = 1, N
                           ...
                           IF( .NOT.SKEWFULL.AND.UPPER )THEN
                              JJ = 1
                              LJ = J
                           ELSE IF( .NOT.SKEWFULL.AND..NOT.UPPER )THEN
                              JJ = J
                              LJ = N - J + 1
...
*
*  TYPE is 'ge', 'gb', 'sy','sb', 'sp', 'skewsy', 'tr', 'tb' OR 'tp'.
*
*  Auxiliary routine for test program for Level 2 Blas.
*
*  -- Written on 10-August-1987.
...
*     .. Executable Statements ..
      GEN = TYPE( 1: 1 ).EQ.'g'
      SYM = TYPE( 1: 1 ).EQ.'s'.AND.TYPE( 2: 2 ).NE.'k'
      SKEW = TYPE( 1: 1 ).EQ.'s'.AND.TYPE( 2: 2 ).EQ.'k'
      TRI = TYPE( 1: 1 ).EQ.'t'
