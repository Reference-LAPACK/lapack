...
{
   return API_SUFFIX(cblas_sasum)(*N, X, *incX);
}
...
{
   API_SUFFIX(cblas_saxpby)(*N, *alpha, X, *incX, *beta, Y, *incY);
   return;
}
...
{
   return API_SUFFIX(cblas_sdot)(*N, X, *incX, Y, *incY);
}

float F77_snrm2(const CBLAS_INT *N, const float *X, const CBLAS_INT *incX)
{
   return API_SUFFIX(cblas_snrm2)(*N, X, *incX);
}
...
CBLAS_INT F77_isamax(const CBLAS_INT *N, const float *X, const CBLAS_INT *incX)
{
   ...
   return (API_SUFFIX(cblas_isamax)(*N, X, *incX)+1);
}
