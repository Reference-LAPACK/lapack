...
{
   API_SUFFIX(cblas_caxpby)(*N, alpha, X, *incX, beta, Y, *incY);
   return;
}
...
{
   API_SUFFIX(cblas_cdotc_sub)(*N, X, *incX, Y, *incY, dotc);
   return;
}
...
{
   API_SUFFIX(cblas_cdotu_sub)(*N, X, *incX, Y, *incY, dotu);
   return;
}
