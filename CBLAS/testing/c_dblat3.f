...
*  cblas_dskewsymm   T PUT F FOR NO TEST. SAME COLUMNS.
...
*  cblas_dskewsyr2k  T PUT F FOR NO TEST. SAME COLUMNS.
...
*           Test DGEMM, 01.
  140       IF (CORDER) THEN
     ...
     $                  CC, CS, CT, G, 1 )
            END IF
            GO TO 190
*           Test DSYMM, 02 and DSKEWSYMM, 08.
  150       IF (CORDER) THEN
...
*           Test DSYRK, 05.
  170       IF (CORDER) THEN
     ...
     $                  CC, CS, CT, G, 1 )
            END IF
            GO TO 190
*           Test DSYR2K, 06 and DSKEWSYR2K, 09.
  180       IF (CORDER) THEN
...
10000 FORMAT( ' COLUMN-MAJOR DATA LAYOUT IS TESTED' )
 ...
 9991 FORMAT( ' AMEND DATA FILE OR INCREASE ARRAY SIZES IN PROGRAM',
     $      /' ******* TESTS ABANDONED *******' )
 9990 FORMAT( ' SUBPROGRAM NAME ', A17,' NOT RECOGNIZED', /' ******* T',
     $      'ESTS ABANDONED *******' )
...
10000 FORMAT( ' ', A17,' PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS',
 ...
 9994 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',
     $      '******' )
 9979 FORMAT( ' ', A17,' COLUMN-MAJOR COMPUTATIONAL TESTS:', I9,
     $      ' RUN,', I9, ' FAILED' )
 9978 FORMAT( ' ', A17,' ROW-MAJOR    COMPUTATIONAL TESTS:', I9,
     $      ' RUN,', I9, ' FAILED' )
*
*     End of DCHK1.
*
...
*     .. External Functions ..
      ...
      EXTERNAL           LDE, LDERES
*     .. External Subroutines ..
      EXTERNAL           DMAKE, DMMCH, CDSYMM, CDSKEWSYMM
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     .. Scalars in Common ..
      INTEGER            INFOT, NOUTC
...
*
                  IF(.NOT.SKEWFULL) THEN
                     CALL DMAKE( 'SY', UPLO, ' ', NA, NA, A, NMAX,
     $                           AA, LDA, RESET, ZERO )
                  ELSE
                     CALL DMAKE( 'SK', UPLO, ' ', NA, NA, A, NMAX,
     $                           AA, LDA, RESET, ZERO )
                  END IF
*
                  DO 60 IA = 1, NALF
...
*
     ...
     $                         LDC )
                        ELSE
                           CALL CDSKEWSYMM( IORDER, SIDE, UPLO, M, N,
     $                         ALPHA, AA, LDA, BB, LDB, BETA, CC,
     $                         LDC )
                        END IF
*
...
*     .. External Functions ..
      ...
      EXTERNAL           LDE, LDERES
*     .. External Subroutines ..
      EXTERNAL           DMAKE, DMMCH, CDSYR2K, CDSKEWSYR2K
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     .. Scalars in Common ..
      INTEGER            INFOT, NOUTC
...
*
                        IF(.NOT.SKEWFULL) THEN
                           CALL DMAKE( 'SY', UPLO, ' ', N, N, C,
     $                                 NMAX, CC, LDC, RESET, ZERO )
                        ELSE
                           CALL DMAKE( 'SK', UPLO, ' ', N, N, C,
     $                                 NMAX, CC, LDC, RESET, ZERO )
                        END IF
*
                        NC = NC + 1
...
*
     ...
     $                         LDC )
                        ELSE
                           CALL CDSKEWSYR2K( IORDER, UPLO, TRANS,
...
*
                        ...
                        ELSE
                           IF(.NOT.SKEWFULL) THEN
                              ISAME( 11 ) = LDERES( 'SY', UPLO, N,
     $                                    N, CS, CC, LDC )
                           ELSE
                              ISAME( 11 ) = LDERES( 'SK', UPLO, N,
     $                                    N, CS, CC, LDC )
                           END IF
                        END IF
                        ISAME( 12 ) = LDCS.EQ.LDC
