...
{
   API_SUFFIX(cblas_zaxpby)(*N, alpha, X, *incX, beta, Y, *incY);
   return;
}
...
{
   API_SUFFIX(cblas_zdotc_sub)(*N, X, *incX, Y, *incY, dotc);
   return;
}
...
{
   API_SUFFIX(cblas_zdotu_sub)(*N, X, *incX, Y, *incY, dotu);
   return;
}
