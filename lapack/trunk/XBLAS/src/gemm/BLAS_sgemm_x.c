 ...
 */
{
  static const char routine_name[] = "BLAS_sgemm_x";
  switch (prec) {
  ...
  case blas_prec_extra:{
	...
	for (i = 0; i < m; i++, ci += incci) {
	  ...
	  for (j = 0; j < n; j++, cij += inccij) {
	    c_elem = c_i[cij];
	    head_tmp1 = (double) c_elem *beta_i;
	    tail_tmp1 = 0.0;
	    c_i[cij] = head_tmp1;
	  }
	}
	...
	if (beta_i == 0.0) {
	  ...
	  for (i = 0; i < m; i++, ci += incci, ai += incai) {
		...
		tail_prod = 0.0;
		{
	      ...
	      c_elem = c_i[cij];
	      head_tmp2 = (double) c_elem *beta_i;
	      tail_tmp2 = 0.0;
	      head_tmp1 = head_sum;
	      tail_tmp1 = tail_sum;
	      {
