 ...
 */
{
  static const char routine_name[] = "BLAS_cgemm_c_s_x";
  switch (prec) {
  ...
  case blas_prec_indigenous:{
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0; i < m; i++, ci += incci, ai += incai) {
		...
		{
		  prod[0] = (double) a_elem[0] * b_elem;
		  prod[1] = (double) a_elem[1] * b_elem;
		}
		sum[0] = sum[0] + prod[0];
		sum[1] = sum[1] + prod[1];
	      }
  ...
  case blas_prec_extra:{
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0; i < m; i++, ci += incci, ai += incai) {
		...
		{
		  head_prod[0] = (double) a_elem[0] * b_elem;
		  tail_prod[0] = 0.0;
		  head_prod[1] = (double) a_elem[1] * b_elem;
		  tail_prod[1] = 0.0;
		}
		{
