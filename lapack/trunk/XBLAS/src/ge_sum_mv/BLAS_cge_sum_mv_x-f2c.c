...
extern void FC_FUNC_(blas_cge_sum_mv_x, BLAS_CGE_SUM_MV_X)
  ...
  (int *m, int *n, const void *alpha, const void *a, int *lda, const void *x,
   ...
   int *prec) {
  BLAS_cge_sum_mv_x(blas_colmajor, *m, *n, alpha, a, *lda, x, *incx, beta, b,
		    *ldb, y, *incy, (enum blas_prec_type) *prec);
}
