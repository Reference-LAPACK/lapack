...
*     Determine required workspace
      IFST = 1
      ILST = JW
      CALL ZHGEQZ( 'S', 'V', 'V', JW, 1, JW, A( KWTOP, KWTOP ), LDA,
...
*              Try to deflate eigenvalue
     ...
     $             1 ) ) ) .LE. MAX( ULP*TEMPR, SMLNUM ) ) THEN
*                 Deflatable
                  KWBOT = KWBOT - 1
               ELSE
...
*        Reflect spike back, this will create optimally packed bulges
         A( KWTOP:KWBOT, KWTOP - 1 ) = A( KWTOP, KWTOP - 1 ) *
     $      DCONJG( QC( 1, 1:JW - ND ) )
         DO K = KWBOT - 1, KWTOP, - 1
            CALL ZLARTG( A( K, KWTOP - 1 ), A( K + 1, KWTOP - 1 ), C1,
     $          S1, TEMP )
            A( K, KWTOP - 1 ) = TEMP
            A( K + 1, KWTOP - 1 ) = CZERO
            K2 = MAX( KWTOP, K - 1 )
            CALL ZROT( IHI - K2 + 1, A( K, K2 ), LDA, A( K + 1, K2 ),
     $          LDA, C1, S1 )
            CALL ZROT( IHI - ( K - 1 ) + 1, B( K, K - 1 ), LDB,
     $          B( K + 1, K - 1 ), LDB, C1, S1 )
            CALL ZROT( JW, QC( 1, K - KWTOP + 1 ), 1, QC( 1,
     $          K + 1 - KWTOP + 1 ), 1, C1, DCONJG( S1 ) )
         END DO
...
*     Apply Qc and Zc to rest of the matrix
     ...
     $       A( ISTARTM, KWTOP ), LDA, ZC, LDZC, CZERO, WORK,
     $       KWTOP - ISTARTM )
        CALL ZLACPY( 'ALL', KWTOP - ISTARTM, JW, WORK, KWTOP - ISTARTM,
     $      A( ISTARTM, KWTOP ), LDA )
         CALL ZGEMM( 'N', 'N', KWTOP - ISTARTM, JW, JW, CONE,
     $       B( ISTARTM, KWTOP ), LDB, ZC, LDZC, CZERO, WORK,
     $       KWTOP - ISTARTM )
        CALL ZLACPY( 'ALL', KWTOP - ISTARTM, JW, WORK, KWTOP - ISTARTM,
     $      B( ISTARTM, KWTOP ), LDB )
      END IF
