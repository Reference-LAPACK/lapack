*> \brief \b SLARFT_UT forms the triangular factor T of a block reflector H = I - vtvH
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download SLARFT_UT + dependencies
...
*  ===========
*
*       SUBROUTINE SLARFT_UT( DIRECT, STOREV, N, K, V, LDV, TAU, T, LDT )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> SLARFT_UT forms the triangular factor T of a real block reflector H
...
*     compute the transpose of the T we would normally compute
*
      LQT = DIRF.AND.TCOLV
*
...
*
         CALL SSYRK('Upper', 'Transpose', K, N-K, ONE, V(K+1,1),
     $         LDV, ONE, T, LDT)
*
...
*
         CALL SSYRK('Upper', 'No Transpose', K, N-K, ONE, V(1,K+1),
     $         LDV, ONE, T, LDT)
*
*        Set the diagonals to 1/tau since we first construct T^{-1}
*           Note: we ensured all of these values are non-zero above
...
*
         CALL SSYRK('Lower', 'No Transpose', K, N-K, ONE, V(1,K+1),
     $         LDV, ONE, T, LDT)
*
*        Set the diagonals to 1/tau since we first construct T^{-1}
*           Note: we ensured all of these values are non-zero above
...
*
         CALL SSYRK('Lower', 'Transpose', K, N-K, ONE, V, LDV,
     $         ONE, T, LDT)
*
*        Set the diagonals to 1/tau since we first construct T^{-1}
*           Note: we ensured all of these values are non-zero above
...
*        Compute T = lt(V_2 * V_2**H + T)
*
         CALL SSYRK('Lower', 'No Transpose', K, N-K, ONE, V, LDV,
     $         ONE, T, LDT)
*
*        Set the diagonals to 1/tau since we first construct T^{-1}
*           Note: we ensured all of these values are non-zero above
...
*        Compute T = ut(V_2 * V_2**H + T)
*
         CALL SSYRK('Upper', 'No Transpose', K, N-K, ONE, V, LDV,
     $         ONE, T, LDT)
*
*        Set the diagonals to 1/tau since we first construct T^{-1}
*           Note: we ensured all of these values are non-zero above
