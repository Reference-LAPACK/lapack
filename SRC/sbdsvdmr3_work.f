*> \brief \b SBDSVDMR3_WORK computational core of SBDSVDMR3.
*>
*> \ingroup bdsvdmr3
*>
*> \par Purpose:
...
*>
*> \verbatim
*> computational core of SBDSVDMR3: preprocessing, dqds singular values, MRRR eigenvectors of the Golub-Kahan matrix, extraction of singular vectors.
*> All routines in this file are auxiliary to SBDSVDMR3 and derive
*> from the LAPACK 3.0 (1999) MRRR kernels SLARRV, SLAR1V, SLARRB and
*> SLARRF rather than from the current SSTEMR kernels.
*> \endverbatim
*
      SUBROUTINE SBDSVDMR3_WORK( JOBZ, UPLO, N, D, E, S, U, LDU, VT,
     $              LDVT,
...
*     SLARRV_TGK.  The bundled SLARRB has a bounded-progress guard so an
*     invalid bracket radius returns INFO instead of looping forever.
*     Standard LAPACK dependencies include SLASQ1, SLAMCH, SLANST,
*     SSCAL, and SSWAP.
*
*  Purpose
*  =======
*
*  SBDSVDMR3 computes the SVD of a real N-by-N bidiagonal matrix
...
*  INFO   (output) INTEGER  0 ok; <0 illegal arg; 1 SLASQ1; 2 SLARRV_TGK.
*
*  =====================================================================
*
*     .. Scalar Arguments ..
...
*     .. Local Scalars ..
     ...
     $                   MPOS, NPOS, NSPLIT, NZSV, TBEG, TEND, BDIM
      REAL   BNRM, EPS, RT2, SCL, SFMIN, THRESH, TOL,
     $                   ZNRM, ZE, ZG
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*     .. External Subroutines ..
      EXTERNAL           SBDTGK, SLARRV_TGK, SLASQ1, SSCAL,
     $                   SSWAP, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, REAL, MAX, MOD, SIGN, SQRT
*     ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'SBDSVDMR3_WORK', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
   ...
   12       CONTINUE
         END IF
         CALL SBSORT( N, S, U, LDU, VT, LDVT, WANTZ )
         M = N
         RETURN
      END IF
*
...
*     eigenvector array Z (LDZ=2N) / SLARRV_TGK WORK.
*     INTEGER:  INDISP 2N (ISPLIT); INDBL 2N (IBLOCK); INDIXW 2N (INDEXW);
*     INDISUP 2N (ISUPPZ scratch); INDIWK (SLARRV_TGK IWORK).
*
      INDTGK = 1
      INDE2 = INDTGK + 2*N
      INDGSC = INDE2 + 2*N
...
*
      ...
      END IF
      IF( SCL.NE.ONE ) THEN
         CALL SSCAL( N, SCL, D, 1 )
         CALL SSCAL( N-1, SCL, E, 1 )
         BNRM = BNRM*SCL
      END IF
*
...
*     Instead the perturbation is applied SELECTIVELY by SLARRF_TGK, and
...
*
      CALL SBDTGK( UPLO, N, D, E, WORK( INDTGK ), WORK( INDE2 ),
     $             WORK( INDGSC ) )
*
...
*     (= singular values) with SLASQ1.  Block of dimension BDIM has its
...
*           even block: square KB-by-KB, KB-1 off-diagonals
   ...
   32       CONTINUE
            CALL SLASQ1( KB, WORK( IDBL ), WORK( IEBL ), WORK( IQ ),
     $                   IINFO )
         ELSE
*           odd block: append a zero diagonal -> (KB+1)-square, KB offdiag
            ...
            DO 34 L = 1, KB
               WORK( IEBL+L-1 ) = ABS( WORK( INDE2+TBEG-2+2*L ) )
   34       CONTINUE
            CALL SLASQ1( KB+1, WORK( IDBL ), WORK( IEBL ), WORK( IQ ),
     $                   IINFO )
         END IF
         IF( IINFO.NE.0 ) THEN
            INFO = 1
            RETURN
         END IF
*        SLASQ1 returns the singular values in DECREASING order in IDBL;
...
*
      IF( .NOT.WANTZ ) THEN
         DO 65 I = 1, NZSV
            S( MPOS+I ) = ZERO
   65    CONTINUE
         CALL SBSORT( N, S, U, LDU, VT, LDVT, .FALSE. )
         IF( SCL.NE.ONE )
     $      CALL SSCAL( N, ONE / SCL, S, 1 )
         M = N
         WORK( 1 ) = SROUNDUP_LWORK( LWMIN )
         IWORK( 1 ) = LIWMIN
         RETURN
      END IF
*
*     NOTE: this must run BEFORE SLARRV_TGK, which overwrites the
...
*
      IF( MPOS.GT.0 ) THEN
         TOL = MAX( REAL( 2*N )*EPS, ZERO )
         CALL SLARRV_TGK( 2*N, WORK( INDTGK ), WORK( INDE2 ),
...
*
  ...
  170 CONTINUE
      M = N
      CALL SBSORT( N, S, U, LDU, VT, LDVT, .TRUE. )
      IF( SCL.NE.ONE )
     $   CALL SSCAL( N, ONE / SCL, S, 1 )
*
      WORK( 1 ) = SROUNDUP_LWORK( LWMIN )
...
*     =================================================================
*
      SUBROUTINE SBSORT( N, S, U, LDU, VT, LDVT, WANTZ )
*
...
*
   ...
   10    CONTINUE
         IF( K.NE.I ) THEN
            ...
            S( I ) = P
            IF( WANTZ ) THEN
               CALL SSWAP( N, U( 1, I ), 1, U( 1, K ), 1 )
               CALL SSWAP( N, VT( I, 1 ), LDVT, VT( K, 1 ), LDVT )
            END IF
         END IF
   20 CONTINUE
      RETURN
