*> \brief \b ZGECXX computes a CX factorization of a real M-by-N matrix A using a truncated (rank k) Householder QR factorization with column pivoting.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download ZGECXX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zgecxx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zgecxx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zgecxx.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE ZGECXX( FACT, USESD, M, N,
...
*       COMPLEX*16          A( LDA, * ), C( LDC, * ), QRC( LDQRC, * ),
*      $                    TAU( * ), WORK( * ), X( LDX, *)
*      ..
*
*
...
*> \verbatim
*>
*> ZGECXX computes a CX factorization of a real M-by-N matrix A using
*> a truncated rank-K Householder QR factorization with a column
*> pivoting algorithm, which is implemented in the ZGEQP3RK routine.
*>
...
*>                preselected columns using the routine ZGEQRF.
*>
...
*>                column pivoting using the routine ZGEQP3RK on
...
*> \param[out] TAU
*> \verbatim
*>          TAU is COMPLEX*16 array, dimension (min(M_sub,N_sub))
...
*>          LWORK >= max( 1, min(M,N) + N ) would be sufficient for all
...
*>
*>          If LWORK = -1, or LRWORK, or LIWORK =-1 then a workspace
*>          query is assumed. The routine only calculates the optimal
*>          size of the WORK, RWORK and IWORK arrays, returns these
*>          values as the first entry of the WORK, RWORK, and IWORK
...
*>          also on:
*>           1a) column preselection stage using ZGEQRF:
*>               the optimal block size NB, the crossover point NX
*>               returned by ILAENV for the routine ZGEQRF
*>               in comparison to N_sel. (For N_sel <= NX
*>               or N_sel <= NB, unblocked code is used in ZGEQRF.)
*>           1b) column preselection stage using ZUNMQR:
*>               the optimal block size NB returned by ILAENV for
*>               the routine ZUNMQR in comparison to N_sel. (For
*>               N_sel <= NB, unblocked code is used in ZUNMQR.)
*>            2) column selection stage via criteria using ZGEQRP3RK:
*>               the optimal block size NB, the crossover point NX
*>               returned by ILAENV for the routine ZGEQRP3RK
*>               in comparison to min(M,N_sel). (For
*>               min(M_sub, N_free, KMAXFREE) <= NX
*>               or min(M_sub, N_free, KMAXFREE) <= NB, unblocked code
*>               is used in ZGEQRP3RK.)
*>           3a) computation of the factor X using ZGEQRF in ZGELS:
*>               the optimal block size NB, the crossover point NX
*>               returned by ILAENV for the routine ZGEQRF
*>               in comparison to K. (For K <= NX or K <= NB,
*>               unblocked code is used in ZGEQRF inside ZGELS.)
*>           3b) computation of the factor X using ZUNMQR in ZGELS:
*>               the optimal block size NB returned by ILAENV for
*>               the routine ZUNMQR in comparison to N. (For
*>               N <= NB, unblocked code is used in ZUNMQR
*>               inside ZGELS.)
*> \endverbatim
*>
*> \param[out] RWORK
...
*>
*>          Minimal LRWORK workspace general requirement.
*>          LRWORK >= max( 1, 2*N ) would be sufficient for all values
*>          of FACT and USESD flags.
*>
*>          The optimal LRWORK is the same as the minimal LRWORK.
*>          The user can still query the routine for the optimal LRWORK.
*>
*>          If LWORK =-1, or LRWORK = -1, or LWORK = -1, then
*>          a workspace query is assumed. The routine only calculates
*>          the optimal size of the WORK, RWORK, and IWORK arrays,
*>          returns these values as the first entry of the WORK, RWORK,
*>          and IWORK arrays respectively, and no error message related
*>          to LRWORK is issued by XERBLA.
*>
*>          Exact minimal workspace requirements.
...
*     .. Local Scalars ..
     ...
     $                   KMAXLS, KP0, LIWKMIN, LIWKOPT, LRWKMIN,
     $                   LRWKOPT, LWKMIN, LWKOPT, MFREE, MDESEL, MINMN,
     $                   MINMNFREE, MRESID, MSUB, NFREE, NDESEL, NRESID,
     $                   NSEL, NSUB
      DOUBLE PRECISION   ABSTOLFREE, EPS, MAXC2NRM, MAXC2NRMKFREE,
     $                   RELTOLFREE, RELMAXC2NRMKFREE, SAFMIN

*     .. External Subroutines ..
      EXTERNAL           ZCOPY, ZGELS, ZGEQP3RK, ZGEQRF, ZLACPY,
     $                   ZUNMQR, ZSWAP, XERBLA
*     ..
*     .. External Functions ..
      LOGICAL            DISNAN, LSAME
      INTEGER            IZAMAX, ILAENV
      DOUBLE PRECISION   DLAMCH, ZLANGE, DZNRM2
      EXTERNAL           DISNAN, DLAMCH, ZLANGE, DZNRM2, IZAMAX,
     $                   ILAENV, LSAME
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL ZGEQRF( MSUB, NSEL, A, LDA, TAU, WORK,
     $                         -1, IINFO )
...
*                     = max( 1, MINMN + N ) for the call of ZGELS.
*
...
*
               KMAXLS = MINMN
*
               CALL ZGELS( 'N', M, KMAXLS, N, QRC, LDQRC, X, LDX,
     $                     WORK, -1, IINFO )
               LWKOPT = MAX( LWKOPT, INT( WORK(1) ) )
*
            END IF
...
*
         IF( ( LWORK.LT.LWKMIN ) .AND. .NOT.LQUERY ) THEN
            INFO = -26
         ELSE IF( ( LRWORK.LT.LRWKMIN ) .AND. .NOT.LQUERY ) THEN
            INFO = -28
...
*
                  CALL ZSWAP( N, A( I, 1 ), LDA, A( MSUB, 1 ), LDA )
*
...
*
                  CALL ZSWAP( M, A( 1, J ), 1, A( 1, NSEL ), 1 )
                  JPIV( J ) = JPIV( NSEL )
                  JPIV( NSEL ) = J
...
*
                  CALL ZSWAP( M, A( 1, J ), 1, A( 1, JDESEL ), 1 )
                  ITEMP = JPIV( J )
                  JPIV( J ) = JPIV( JDESEL )
                  JPIV( JDESEL ) = ITEMP
...
*              A_sub_resid(NSEL) = A(NSEL+1:MSUB, NSEL+1:NSUB)
*
         CALL ZGEQRF( MSUB, NSEL, A, LDA, TAU, WORK, LWORK, IINFO )
*
...
*
      IF( MIN( MRESID, NRESID ).NE.0 ) THEN
         FNRMK = ZLANGE( 'F',  MRESID, NRESID, A( K+1, K+1 ),
     $                   LDA, WORK )
      ELSE
         FNRMK = ZERO
      END IF
...
*
         DO J = 1, K, 1
            CALL ZCOPY( M, X( 1, JPIV( J ) ), 1, C( 1, J ), 1 )
         END DO
*
      ELSE
*
...
*        of the routine.
*
         CALL ZLACPY( 'F', M, K, C, LDC, QRC, LDQRC )
*
         CALL ZGELS( 'N', M, K, N, QRC, LDQRC, X, LDX,
     $               WORK, LWORK, IINFO )
         INFO = IINFO
*
      END IF
