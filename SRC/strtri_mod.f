...
*
*> Download STRTRI_MOD + dependencies
...
*  ===========
*
*       RECURSIVE SUBROUTINE STRTRI_MOD( UPLO, DIAG, N, A, LDA, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          DIAG, UPLO
*       INTEGER            INFO, LDA, N
...
*> \verbatim
*>
*> STRTRI_MOD computes the inverse of a real upper or lower triangular
*> matrix A.
*>
...
*     .. External Subroutines ..
      EXTERNAL XERBLA, STRTI2_MOD, STRMM, STRSM_MOD
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      ...
      IF( N.LE.NX ) THEN
         CALL STRTI2_MOD(UPLO, DIAG, N, A, LDA, INFO)
         RETURN
      END IF
      ...
      IF( UPPER ) THEN
         ! Compute X_{22}
         CALL STRTRI_MOD('Upper', DIAG, N-K, A(K+1,K+1), LDA, INFO)
         ! Propogate to A_{12}
         CALL STRMM('Right', 'Upper', 'No Transpose', DIAG, K, N-K,
     $      -ONE, A(K+1,K+1), LDA, A(1,K+1), LDA)
         ! Solve for X_{12}
         CALL STRSM_MOD('Left', 'Upper', 'No Transpose', DIAG,
     $      K, N-K, ONE, A, LDA, A(1,K+1), LDA)
         ! Solve for X_{11}
         CALL STRTRI_MOD('Upper', DIAG, K, A, LDA, INFO)
      ELSE ! A is lower
         ! Compute X_{11}
         CALL STRTRI_MOD('Lower', DIAG, K, A, LDA, INFO)
         ! Propogate to A_{21}
         CALL STRMM('Right', 'Lower', 'No Transpose', DIAG, N-K, K,
     $      -ONE, A, LDA, A(K+1,1), LDA)
         ! Solve for X_{21}
         CALL STRSM_MOD('Left', 'Lower', 'No Transpose', DIAG,
     $      N-K, K, ONE, A(K+1,K+1), LDA, A(K+1,1), LDA)
         ! Compute X_{22}
         CALL STRTRI_MOD('Lower', DIAG, N-K, A(K+1,K+1), LDA, INFO)
      END IF
