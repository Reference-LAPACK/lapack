...
*
      IF(K.LT.NX) THEN
         CALL ZHTRK(UPLOT, UPLOC, TRANS, DIAG, K, ALPHA, T, LDT,
     $         BETA, C, LDC)
         RETURN
      END IF
*
...
*
      IF(K.EQ.1) THEN
         IF(BETA.EQ.ZERO) THEN
            C(1,1) = ZERO
         ELSE
            C(1,1) = BETA*DBLE(C(1,1))
         END IF
         IF(UNITT) THEN
            C(1,1) = ALPHA + C(1,1)
         ELSE
            C(1,1) = ALPHA*DBLE(T(1,1)*DCONJG(T(1,1))) + C(1,1)
         END IF
         RETURN
      END IF
*
...
*
            CALL ZHERK(UPLOC, TRANS, K-L, L, ALPHA, T(1,L+1), LDT,
     $            ONE, C(L+1,L+1), LDC)
            IF(UPPERC) THEN
*
*              Compute C_{1,2} = \alpha*T_{1,1}**H*T_{1,2} + \beta C_{1,2} (TRMMOOP)
*
               CALL ZTRMMOOP('Left', UPLOT, 'Conjugate',
...
*
               CALL ZTRMMOOP('Right', UPLOT, 'No Transpose',
...
*
            CALL ZHERK(UPLOC, TRANS, L, K-L, ALPHA, T(L+1,1), LDT,
     $            ONE, C, LDC)
*
...
*
            CALL ZHERK(UPLOC, TRANS, L, K-L, ALPHA, T(1,L+1), LDT,
     $            ONE, C, LDC)
*
...
*
               CALL ZTRMMOOP('Right', UPLOT, 'Conjugate',
     $               'No Transpose', DIAG, L, K-L, DCMPLX(ALPHA),
...
*
               CALL ZTRMMOOP('Left', UPLOT, 'No Transpose',
...
*
            CALL ZHERK(UPLOC, TRANS, K-L, L, ALPHA, T(L+1,1), LDT,
     $            ONE, C(L+1,L+1), LDC)
            IF(UPPERC) THEN
*
