...
*
*> \htmlonly
*> Download CLAORHR + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/claorhr.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/claorhr.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/claorhr.f">
*> [TXT]</a>
*>
*  Definition:
*  ===========
*
*       SUBROUTINE CLAORHR( M, N, MB1, NB1, A, LDA, T1, LDT1, NB2, T2,
...
*> \verbatim
*>
*> CLAORHR is an auxiliary routine of CORHR. CLAORHR is a low-level
*> routine that does the computation. CLAORHR performs Householder
...
*> QR factorization routine CLATSQR. The reconstructed orthogonal M-by-M
...
*> output in A and T from QR factorization routine CGEQRT. CLAORHR uses
...
*> \param[out] W
*> \verbatim
*>          W is (workspace) COMPLEX array, dimension (LDW, N)
*>          This workspace is used to store array C in the call to
*>          the CLAMTSQR routine.
*> \endverbatim
*>
*> \param[in] LDW
*> \verbatim
...
*>          the CLAMTSQR routine.
*>          See the documentation of CLAMTSQR.
*> \endverbatim
*>
*> \param[in] LWORK
*> \verbatim
...
*>          by XERBLA. See the documentation of CLAMTSQR.
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*> =========
*>
*> See the documentation of CLATSQR.
*> On entry in A, the output from the M-by-N matrix A from CLATSQR is
...
*> by CLATSQR. The elements on and above the diagonal represent the
*> upper-triangular N-by-N matrix R_in computed by CLATSQR.
*>
...
*>                   = CEIL(N/NB1)
*>
*> On entry in T1, the output from the matrix T from CLATSQR. The matrix
...
*> size MB respectively used in CLATSQR.
*>
*> ========
*> On exit:
*> ========
...
*> as the output from the CGEQRT routine stored in the matrices A and T.
*> See the documentation for CGEQRT.
*>
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CLAORHR_GETRFNP, CLAMTSQR, CLASET,
     $                   CSCAL, CTRSM, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          CMPLX, MIN
...
*
         NB1LOCAL = MIN( NB1, N )
*
*        WORK in CLAORHR is only used as a work array in
*        the call to CLAMTSQR, therefore the optimal size LW for WORK
*        is the same as the optimal size for WORK in CLAMTSQR.
*
         LW = N * NB1LOCAL
*
...
*     and zeros elsewhere.
*
      CALL CLASET( 'F', M, N, CZERO, CONE, W, LDW )
*
*     (1b)  On input, W stores ( I ); on output, W stores Q1_in.
*                              ( 0 )
*
      CALL CLAMTSQR( 'L', 'N', M, N, N, MB1, NB1LOCAL, A, LDA, T1, LDT1,
...
*
      CALL CLAORHR_GETRFNP( N, N, W, LDW, D, IINFO )
*
*     (2-2) Solve for L21.
*
      CALL CTRSM( 'R', 'U', 'N', 'N', M-N, N, CONE, W, LDW,
     $            W( N+1, 1 ), LDW )
*
...
*
      DO J = 1, N
         CALL CCOPY( M-J, W( J+1, J ), 1, A( J+1, J ), 1 )
      END DO
*
...
*
         JBTEMP1 = JB - 1
         DO J = JB, JB+JNB-1
            CALL CCOPY( J-JBTEMP1, W( JB, J ), 1, T2( 1, J ), 1 )
         END DO
*
...
*
         DO J = JB, JB+JNB-1
            IF( D( J ).EQ.CONE ) THEN
               CALL CSCAL( J-JBTEMP1, -CONE, T2( 1, J ), 1 )
            END IF
         END DO
*
...
*
      DO I = 1, N
         IF( D( I ).EQ.-CONE ) THEN
            CALL CSCAL( NPLUSONE-I, -CONE, A( I, I ), LDA )
         END IF
      END DO
*
      WORK( 1 ) = CMPLX( LW )
