...
*> \verbatim
*>
*> CHETRRK performs one of the symmetric rank k operations
*>
...
*> and A is an k by k either support or lower triangular matrix
*> \endverbatim
*
*  Arguments:
*  ==========
...
*
      NX = ILAENV(3, 'CHETRRK', UPLOA // UPLOC // TRANS // DIAG,
     $      K, -1, -1, -1)
*
      IF(K.LT.NX.OR.K.EQ.1) THEN
         CALL CHETRRK_LVL2(UPLOA, UPLOC, TRANS, DIAG, K,
     $         ALPHA, A, LDA, BETA, C, LDC)
         RETURN
      END IF
*
...
*
      ...
      UNITT = LSAME(DIAG,'U')
*
*     Convert alpha and beta to complex to pass to TRMMOOP
*
...
*                      |-----------------------| |-----------------|         |-----------------|
*
*           Which gives us the following component-wise representation of C
*
...
*
            CALL CHETRRK(UPLOA, UPLOC, TRANS, DIAG, L, ALPHA,
     $            A, LDA, BETA, C, LDC)
*
*           Compute C_{2,2}
*           C_{2,2} = \alpha*A_{2,2}**H*A_{2,2} + \beta C_{2,2}
*
            CALL CHETRRK(UPLOA, UPLOC, TRANS, DIAG, K-L, ALPHA,
     $            A(L+1,L+1), LDA, BETA, C(L+1,L+1), LDC)
*
*           C_{2,2} = \alpha*A_{1,2}**H*A_{1,2} + C_{2,2}
*
            CALL CHERK(UPLOC, TRANS, K-L, L, ALPHA, A(1,L+1), LDA,
     $            ONE, C(L+1,L+1), LDC)
            IF(UPPERC) THEN
*
...
*
     ...
     $               'No Transpose', DIAG, L, K-L, CALPHA, A, LDA,
     $               A(1, L+1), LDA, CBETA, C(1,L+1), LDC)
            ELSE
*
*              Compute C_{2,1} = \alpha*A_{1,2}**H*A_{1,1} + \beta C_{2,1} (TRMMOOP)
*
               CALL CTRMMOOP('Right', UPLOA, 'No Transpose',
     $               'Conjugate', DIAG, K-L, L, CALPHA, A, LDA,
     $               A(1, L+1), LDA, CBETA, C(L+1,1), LDC)
            END IF
         ELSE
*
...
*
            CALL CHERK(UPLOC, TRANS, L, K-L, ALPHA, A(L+1,1), LDA,
     $            ONE, C, LDC)
*
...
*
               CALL CTRMMOOP('Right', UPLOA, 'No Transpose',
     $               'Conjugate', DIAG, L, K-L, CALPHA, A(L+1,L+1), LDA,
     $                A(L+1,1), LDA, CBETA, C(1,L+1), LDC)
            ELSE
*
*              Compute C_{2,1} = \alpha*A_{2,2}**H*A_{2,1} + \beta*C_{2,1}
*
               CALL CTRMMOOP('Left', UPLOA, 'Conjugate',
     $               'No Transpose', DIAG, K-L, L, CALPHA,
     $               A(L+1,L+1), LDA, A(L+1, 1), LDA, CBETA,
     $               C(L+1,1), LDC)
            END IF
         END IF
      ELSE
...
*
            CALL CHERK(UPLOC, TRANS, L, K-L, ALPHA, A(1,L+1), LDA,
     $            ONE, C, LDC)
*
...
*
     ...
     $               'No Transpose', DIAG, L, K-L, CALPHA,
     $               A(L+1,L+1), LDA, A(1, L+1), LDA, CBETA,
     $               C(1,L+1), LDC)
            ELSE
*
...
*
            CALL CHETRRK(UPLOA, UPLOC, TRANS, DIAG, L, ALPHA, A, LDA,
     $            BETA, C, LDC)
*
*           Compute C_{2,2}
*           C_{2,2} = \alpha*A_{2,2}*A_{2,2}**H + \beta*C_{2,2}
...
*
            CALL CHERK(UPLOC, TRANS, K-L, L, ALPHA, A(L+1,1), LDA,
     $            ONE, C(L+1,L+1), LDC)
            IF(UPPERC) THEN
*
...
*
               CALL CTRMMOOP('Left', UPLOA, 'No Transpose',
     $               'Conjugate', DIAG, L, K-L, CALPHA, A, LDA,
     $               A(L+1,1), LDA, CBETA, C(1,L+1), LDC)
            ELSE
*
...
*
               CALL CTRMMOOP('Right', UPLOA, 'Conjugate',
     $               'No Transpose', DIAG, K-L, L, CALPHA, A, LDA,
     $               A(L+1,1), LDA, CBETA, C(L+1,1), LDC)
            END IF
         END IF
      END IF
      END SUBROUTINE
