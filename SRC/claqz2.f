...
*        Reflect spike back, this will create optimally packed bulges
     ...
     $      CONJG( QC( 1, 1:JW - ND ) )
         DO K = KWBOT - 1, KWTOP, - 1
            CALL CLARTG( A( K, KWTOP - 1 ), A( K + 1, KWTOP - 1 ), C1,
     $          S1, TEMP )
            A( K, KWTOP - 1 ) = TEMP
            A( K + 1, KWTOP - 1 ) = CZERO
            K2 = MAX( KWTOP, K - 1 )
            CALL CROT( IHI - K2 + 1, A( K, K2 ), LDA, A( K + 1, K2 ),
     $          LDA, C1, S1 )
            CALL CROT( IHI - ( K - 1 ) + 1, B( K, K - 1 ), LDB,
     $          B( K + 1, K - 1 ), LDB, C1, S1 )
            CALL CROT( JW, QC( 1, K - KWTOP + 1 ), 1, QC( 1,
     $          K + 1 - KWTOP + 1 ), 1, C1, CONJG( S1 ) )
         END DO
