*> \brief \b SLARF0C2 applies an elementary reflector to a rectangular matrix
...
*  ===========
*
*     SUBROUTINE SLARF0C2( C2JOB, SIDE, DIRECT, STOREV, M, N, TAU,
*                          V, C, LDC )
*
*     .. Scalar Arguments ..
*     INTEGER           M, N, LDC
...
*>          TAU must contain the value TAU returned from SLARFG
*> \endverbatim
*>
*> \param[in] V
*> \verbatim
...
*>                   (  1 )
*>
*>  Also see slarf1f and slarf1l along with their usage in org{2r,2l,r2,l2}
...
*     .. External Subroutines ..
      EXTERNAL          SGEMV, SGER, SLASET, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC         MAX
*     ..
...
*
      ...
      IF( .NOT.(SIDEL.OR.SIDER) ) THEN
         CALL XERBLA('SLARF0C2', 2) ! For consistency with slarfb0c2
      END IF
*
...
*
            IF ( C2I ) THEN
               CALL SLASET('All', M-1, N, ZERO, ONE, C(2,1), LDC)
            END IF
         ELSE
            IF( C2I ) THEN
*
...
*
               CALL SGEMV('Transpose', M-1, N, -TAU, C(2,1), LDC,
     $              V, INCV, ZERO, C, LDC)
*
*              C2 = C2 + v*C1
*
               CALL SGER(M-1, N, ONE, V, INCV, C, LDC, C(2,1), LDC)
            END IF
         END IF
      ELSE IF( LQ ) THEN
*
...
*
            IF ( C2I ) THEN
               CALL SLASET('All', M, N-1, ZERO, ONE, C(1,2), LDC)
            END IF
         ELSE
            IF( C2I ) THEN
*
...
*              C1 = -tau*C2*v'
*
               CALL SGEMV('No Transpose', M, N-1, -TAU, C(1,2), LDC,
     $            V, INCV, ZERO, C, 1)
*
*              C2 = C2 + C1*v
*
               CALL SGER(M, N-1, ONE, C, 1, V, INCV, C(1,2), LDC)
            END IF
         END IF
      ELSE IF( QL ) THEN
*
...
*
            IF( C2I ) THEN
               CALL SLASET('All', M-1, N, ZERO, ZERO, C, LDC)
               DO I = 1, N
                  C(M-N-1+I,I) = ONE
               END DO
            END IF
...
*              C1 = -tau*C2'*v
*
               CALL SGEMV('Transpose', M-1, N, -TAU, C, LDC,
     $               V, INCV, ZERO, C(M,1), LDC)
*
*              C2 = C2 + v*C1
*
               CALL SGER(M-1, N, ONE, V, INCV, C(M,1), LDC, C, LDC)
            END IF
         END IF
      ELSE IF( RQ ) THEN
*
...
*
            IF( C2I ) THEN
               CALL SLASET('All', M, N-1, ZERO, ZERO, C, LDC)
               DO I = 1, M
                  C(I, N-M-1+I) = ONE
               END DO
            END IF
...
*              C1 = -tau*C2v'
*
               CALL SGEMV('No Transpose', M, N-1, -TAU, C, LDC,
     $            V, INCV, ZERO, C(1,N), 1)
*
*              C2 = C2 + C1*v
*
               CALL SGER(M, N-1, ONE, C(1,N), 1, V, INCV, C, LDC)
            END IF
         END IF
      END IF
      RETURN
