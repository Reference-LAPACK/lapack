*> \brief \b SBDTGK forms the Golub-Kahan tridiagonal matrix of a bidiagonal matrix.
*>
*> \ingroup bdsvdmr3
*>
*> \par Purpose:
...
*> \endverbatim
*
      SUBROUTINE SBDTGK( UPLO, N, D, E, DTGK, ETGK, GERSCH )
*
...
*  SBDTGK forms the symmetric tridiagonal Golub-Kahan matrix T_GK of
...
*  SLAR1V_TGK to restrict the twist-index search); since the diagonal is
...
*
*  DTGK   (output) REAL array, dimension (2*N)
*         The diagonal of T_GK (all zeros).
*
*  ETGK   (output) REAL array, dimension (2*N-1)
*         The off-diagonal beta_i of T_GK (interleaved D and E).
*
*  GERSCH (output) REAL array, dimension (2*(2*N))
