*> \brief \b SLAED9_BR used by SSTEBR.
*
*  SLAED9_BR finds the roots of the secular equation for one
*  boundary-row DC merge and directly updates a small set of selected
*  rows.  It mirrors SLAED9's root solve and eigenvector normalization,
*  but avoids materializing the full K-by-K secular eigenvector block.
*  SLAED4_BR returns high-relative-accuracy DELTA values for each root;
...
*
      SUBROUTINE SLAED9_BR( K, NBR, D, QIN, LDQIN, QOUT, LDQOUT,
...
*     .. Local Scalars ..
      ...
      REAL               ACC1, ACC2, DLT, ROWSUM, TEMP
*     ..
*     .. External Subroutines ..
      EXTERNAL           SLAED4, SLAED4_BR, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( NBR.EQ.0 ) THEN
         DO 5 J = 1, K
            CALL SLAED4( K, J, DLAMBDA, W, DELTA, RHO, D( J ), INFO )
            IF( INFO.NE.0 )
     $         GO TO 170
    5    CONTINUE
         GO TO 170
      END IF
*
*     For K = 1 or 2, SLAED4/SLAED5 returns normalized eigenvectors
*     directly in DELTA.  Keep this path identical to SLAED9's
*     special-case copy to S.
*
      IF( K.EQ.1 .OR. K.EQ.2 ) THEN
         DO 30 J = 1, K
            CALL SLAED4( K, J, DLAMBDA, W, DELTA, RHO, D( J ),
     $                   INFO )
            IF( INFO.NE.0 )
     $         GO TO 170
            IF( NBR.EQ.2 ) THEN
...
*     accumulate the product needed to reconstruct SLAED9's updated
*     W.  The high-relative-accuracy DELTA values produced by
*     SLAED4_BR are consumed immediately rather than reconstructed
...
*
      DO 44 J = 1, K
         CALL SLAED4_BR( K, J, DLAMBDA, WSGN, DELTA, RHO, D( J ),
     $                   IORG, TAU( J ), NEAR( 1, J ),
     $                   NEAR( 2, J ), INFO )
         IF( INFO.NE.0 )
     $      GO TO 170
