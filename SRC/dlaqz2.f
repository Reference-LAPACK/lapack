...
*
     ...
     $       TEMP )
         A( IHI - 1, IHI - 2 ) = TEMP
         A( IHI, IHI - 2 ) = ZERO
         CALL DROT( ISTOPM - IHI + 2, A( IHI - 1, IHI - 1 ), LDA,
     $       A( IHI, IHI - 1 ), LDA, C1, S1 )
         CALL DROT( ISTOPM - IHI + 2, B( IHI - 1, IHI - 1 ), LDB,
...
*
         CALL DROT( K + 3 - ISTARTM + 1, A( ISTARTM, K + 2 ), 1,
     $       A( ISTARTM, K + 1 ), 1, C1, S1 )
         CALL DROT( K + 3 - ISTARTM + 1, A( ISTARTM, K + 1 ), 1,
     $       A( ISTARTM, K ), 1, C2, S2 )
         CALL DROT( K + 2 - ISTARTM + 1, B( ISTARTM, K + 2 ), 1,
     $       B( ISTARTM, K + 1 ), 1, C1, S1 )
         CALL DROT( K + 2 - ISTARTM + 1, B( ISTARTM, K + 1 ), 1,
     $       B( ISTARTM, K ), 1, C2, S2 )
         IF ( ILZ ) THEN
            CALL DROT( NZ, Z( 1, K + 2 - ZSTART + 1 ), 1, Z( 1,
...
*
         CALL DROT( ISTOPM - K, A( K + 2, K + 1 ), LDA, A( K + 3,
     $       K + 1 ), LDA, C1, S1 )
         CALL DROT( ISTOPM - K, A( K + 1, K + 1 ), LDA, A( K + 2,
     $       K + 1 ), LDA, C2, S2 )
*
         CALL DROT( ISTOPM - K, B( K + 2, K + 1 ), LDB, B( K + 3,
     $       K + 1 ), LDB, C1, S1 )
         CALL DROT( ISTOPM - K, B( K + 1, K + 1 ), LDB, B( K + 2,
     $       K + 1 ), LDB, C2, S2 )
         IF ( ILQ ) THEN
            CALL DROT( NQ, Q( 1, K + 2 - QSTART + 1 ), 1, Q( 1,
     $          K + 3 - QSTART + 1 ), 1, C1, S1 )
            CALL DROT( NQ, Q( 1, K + 1 - QSTART + 1 ), 1, Q( 1,
