...
*
     ...
     $      c2,s2)
         if (ilz) then
            call drot(nz,Z(1,ihi-zStart+1),1,Z(1,ihi-1-zStart+1),1,c1,
     $         s1)
            call drot(nz,Z(1,ihi-1-zStart+1),1,Z(1,ihi-2-zStart+1),1,c2,
     $         s2)
         end if
*
         ...
         A(ihi-1,ihi-2) = temp
         A(ihi,ihi-2) = zero
         call drot(istopm-ihi+2,A(ihi-1,ihi-1),ldA,A(ihi,ihi-1),ldA,c1,
     $      s1)
         call drot(istopm-ihi+2,B(ihi-1,ihi-1),ldB,B(ihi,ihi-1),ldB,c1,
     $      s1)
         if (ilq) then
            call drot(nq,Q(1,ihi-1-qStart+1),1,Q(1,ihi-qStart+1),1,c1,
     $         s1)
         end if
*
         ...
         B(ihi,ihi) = temp
         B(ihi,ihi-1) = zero
         call drot(ihi-istartm,B(istartm,ihi),1,B(istartm,ihi-1),1,c1,
     $      s1)
         call drot(ihi-istartm+1,A(istartm,ihi),1,A(istartm,ihi-1),1,c1,
     $      s1)
         if (ilz) then
...
*
         call drot(k+3-istartm+1,A(istartm,k+2),1,A(istartm,k+1),1,c1,
     $      s1)
         call drot(k+3-istartm+1,A(istartm,k+1),1,A(istartm,k),1,c2,s2)
         call drot(k+2-istartm+1,B(istartm,k+2),1,B(istartm,k+1),1,c1,
     $      s1)
