...
*  ===========
*
*       SUBROUTINE DLAORHR( M, N, MB1, NB1, A, LDA, T1, LDT1, NB2, T2,
...
*> \verbatim
*>
*> DLAORHR is an auxiliary routine of DORHR. DLAORHR is a low-level
*> routine that does the computation. DLAORHR performs Householder
...
*> output in A and T from QR factorization routine DGEQRT. DLAORHR uses
...
*>          by XERBLA. See the documentation of DLAMTSQR.
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*> =========
*>
*> See the documentation of DLATSQR.
*> On entry in A, the output from the M-by-N matrix A from DLATSQR is
...
*> by DLATSQR. The elements on and above the diagonal represent the
...
*>                   = CEIL(N/NB1)
*>
*> On entry in T1, the output from the matrix T from DLATSQR. The matrix
...
*> size MB respectively used in DLATSQR.
*>
*> ========
*> On exit:
*> ========
...
*> as the output from the DGEQRT routine stored in the matrices A and T.
*> See the documentation for DGEQRT.
*>
...
*
         NB1LOCAL = MIN( NB1, N )
*
*        WORK in DLAORHR is only used as a work array in
*        the call to DLAMTSQR, therefore the optimal size LW for WORK
*        is the same as the optimal size for WORK in DLAMTSQR.
*
         LW = N * NB1LOCAL
*
...
*     and zeros elsewhere.
*
      CALL DLASET( 'F', M, N, ZERO, ONE, W, LDW )
*
*     (1b)  On input, W stores ( I ); on output, W stores Q1_in.
*                              ( 0 )
*
      CALL DLAMTSQR( 'L', 'N', M, N, N, MB1, NB1LOCAL, A, LDA, T1, LDT1,
...
*
      CALL DLAORHR_GETRFNP( N, N, W, LDW, D, IINFO )
*
*     (2-2) Solve for L21.
*
      CALL DTRSM( 'R', 'U', 'N', 'N', M-N, N, ONE, W, LDW,
     $            W( N+1, 1 ), LDW )
*
...
*
      DO J = 1, N
         CALL DCOPY( M-J, W( J+1, J ), 1, A( J+1, J ), 1 )
      END DO
*
...
*
         JBTEMP1 = JB - 1
         DO J = JB, JB+JNB-1
            CALL DCOPY( J-JBTEMP1, W( JB, J ), 1, T2( 1, J ), 1 )
         END DO
*
...
*
         DO J = JB, JB+JNB-1
            IF( D( J ).EQ.ONE ) THEN
               CALL DSCAL( J-JBTEMP1, -ONE, T2( 1, J ), 1 )
            END IF
         END DO
*
...
*
      DO I = 1, N
         IF( D( I ).EQ.-ONE ) THEN
            CALL DSCAL( NPLUSONE-I, -ONE, A( I, I ), LDA )
         END IF
      END DO
*
      WORK( 1 ) = DBLE( LW )
