...
*        Reflect spike back, this will create optimally packed bulges
     ...
     $       1:JW - ND )
         DO K = KWBOT - 1, KWTOP, - 1
            CALL SLARTG( A( K, KWTOP - 1 ), A( K + 1, KWTOP - 1 ), C1,
     $          S1, TEMP )
            A( K, KWTOP - 1 ) = TEMP
            A( K + 1, KWTOP - 1 ) = ZERO
            K2 = MAX( KWTOP, K - 1 )
            CALL SROT( IHI - K2 + 1, A( K, K2 ), LDA, A( K + 1, K2 ),
     $          LDA, C1, S1 )
            CALL SROT( IHI - ( K - 1 ) + 1, B( K, K - 1 ), LDB,
     $          B( K + 1, K - 1 ), LDB, C1, S1 )
...
*              Move double pole block down and remove it
               DO K2 = K - 1, KWBOT - 2
                  CALL SLAQZ2( .TRUE., .TRUE., K2, KWTOP, KWTOP + JW -
...
*                 Move shift down
     ...
     $                S1, TEMP )
                  B( K2 + 1, K2 + 1 ) = TEMP
                  B( K2 + 1, K2 ) = ZERO
                  CALL SROT( K2 + 2 - ISTARTM + 1, A( ISTARTM, K2 + 1 ),
