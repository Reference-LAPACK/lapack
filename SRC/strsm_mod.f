...
*  ===========
*
*     RECURSIVE SUBROUTINE STRSM_MOD(SIDE, UPLO, TRANSA, DIAG, M, N,
*    $      ALPHA, A, LDA, B, LDB)
*
*       .. Scalar Arguments ..
*       REAL ALPHA
...
*> \verbatim
*>
*> STRSM_MOD solves one of the matrix equations
*>
...
*
      IF (ALPHA.EQ.ZERO) THEN
         ...
         CALL SLASET('All', M, N, ZERO, ZERO, B, LDB)
         RETURN
      END IF
...
!    $   N, -1, -1, -1 )
      ...
      IF( MIN(M,N).LE.NX ) THEN
         CALL STRSM_LVL2_MOD(SIDE, UPLO, TRANSA, DIAG, M, N,
     $      ALPHA, A, LDA, B, LDB)
         RETURN
      END IF
*
...
*
     ...
     $         M-K, K, ALPHA, A(K+1,K+1), LDA, B(K+1,1), LDB)
               ...
               CALL SGEMM('N', 'N', K, K, M-K, NEG_ONE,
     ...
     $         M-K, N-K, ALPHA, A(K+1,K+1), LDA, B(K+1,K+1), LDB)
               ...
               CALL SGEMM('N', 'N', K, N-K, M-K, NEG_ONE,
...
*
               ...
               ! Compute X_{11}
               CALL STRSM_MOD('left', 'upper', 'transpose', DIAG,
     $            K, K, ALPHA, A, LDA, B, LDB )
               ! Update B_{21}
               CALL SGEMM( 'transpose', 'no transpose', M-K, K, K,
     ...
     $            K, N-K, ALPHA, A, LDA, B(1,K+1), LDB )
               ...
               !Update B_{22}
               CALL SGEMM( 'transpose', 'no transpose', M-K, N-K, K,
...
*
     ...
     $            K, K, ALPHA, A, LDA, B, LDB)
               ...
               ! Update B_{21}
               CALL SGEMM('no transpose', 'no transpose', M-K, K, K,
     $            NEG_ONE, A(K+1,1), LDA, B, LDB, ALPHA, B(K+1,1), LDB)
               ! Solve for X_{21}
               CALL STRSM_MOD('left', 'lower', 'no transpose',
...
*
               ! Compute X_{21}
               CALL STRSM_MOD('left', 'lower', 'transpose', DIAG,
     $            M-K, K, ALPHA, A(K+1,K+1), LDA, B(K+1,1), LDB)

               ! Update B_{11}
               CALL SGEMM('transpose', 'no transpose', K, K, M-K,
     ...
     $            M-K, N-K, ALPHA, A(K+1,K+1), LDA, B(K+1,K+1), LDB)
               ...
               ! Update B_{12}
               CALL SGEMM('transpose', 'no transpose', K, N-K, M-K,
...
*
               ! Solve for X_{11}
               CALL STRSM_MOD(SIDE, UPLO, TRANSA, DIAG, K, K, ALPHA,
     $            A, LDA, B, LDB)
               ! Update B_{12}
               CALL SGEMM('No transpose', 'No transpose', K, N-K, K,
     $            NEG_ONE, B, LDB, A(1,K+1), LDA, ALPHA, B(1,K+1), LDB)
               ! Solve for X_{12}
               CALL STRSM_MOD(SIDE, UPLO, TRANSA, DIAG, K, N-K,
     $            ALPHA, A(K+1,K+1), LDA, B(1,K+1), LDB)
               ! Solve for X_{21}
               CALL STRSM_MOD(SIDE, UPLO, TRANSA, DIAG, M-K, K,
     ...
     $            B(K+1, K+1), LDB)
               ! Solve for X_{22}
               CALL STRSM_MOD('Right', 'Upper', 'No Transpose', DIAG,
     $            M-K, N-K, ONE, A(K+1,K+1), LDA, B(K+1,K+1), LDB)
            ELSE ! A is transposed
*
*              Break A and B apart as
...
*
               ! Compute X_{12}
               CALL STRSM_MOD('Right', 'Upper', 'Transpose', DIAG,
     $            K, N-K, ALPHA, A(K+1,K+1), LDA, B(1, K+1), LDB)
               ! Update B_{11}
               CALL SGEMM('No Transpose','Transpose', K, K, N-K,
     ...
     $            M-K, N-K, ALPHA, A(K+1,K+1), LDA, B(K+1,K+1), LDB)
               ! Update B_{21}
               CALL SGEMM('No Transpose', 'Transpose', M-K, K, K,
...
*
               ! Solve for X_{12}
               CALL STRSM_MOD('Right', 'Lower', 'No Transpose', DIAG,
     $            K, N-K, ALPHA, A(K+1,K+1), LDA, B(1,K+1), LDB)
               ! Update B_{11}
     ...
     $            M-K, N-K, ALPHA, A(K+1,K+1), LDA, B(K+1,K+1), LDB)
               ! Update B_{21}
               CALL SGEMM('No Transpose', 'Transpose', M-K, K, N-K,
...
*
               ! Solve for X_{11}
               CALL STRSM_MOD('Right', 'Lower', 'Transpose', DIAG,
     $            K, K, ALPHA, A, LDA, B, LDB)
               ! Update B_{12}
               CALL SGEMM('No Transpose', 'Transpose', K, N-K, K,
     ...
     $            M-K, K, ALPHA, A, LDA, B(K+1,1), LDB)
               ! Update B_{22}
               CALL SGEMM('No Transpose', 'Transpose', M-K, N-K, K,
