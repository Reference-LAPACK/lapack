*> \brief \b CTRTI2_MOD computes the inverse of a triangular matrix allowing for
...
*> \verbatim
*>
*> CTRTI2_MOD computes the inverse of a real upper or lower triangular
...
*
*  =====================================================================
      SUBROUTINE CTRTI2_MOD( UPLO, DIAG, N, A, LDA, INFO )
      IMPLICIT NONE
*
...
*     .. External Subroutines ..
      EXTERNAL XERBLA, CSCAL, CLASET, CTRSM_MOD
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
         DO J = 1, N
            IF( UNIT ) THEN
               CALL CSCAL(N-J, -ONE, A(J+1,J), 1)
            ELSE
               IF( A(J,J).EQ.ZERO ) THEN
                  ! Set this column to 0
                  CALL CLASET('a', N-J, 1, ZERO, ZERO, A(J+1,J), LDA)
                  !call claset('a', 1, j-1, zero, zero, a(j+1,1), lda)
               ELSE
                  ! In this case we must manually invert the diagonal
                  ! element then propagate
                  A(J,J) = ONE / A(J,J)
                  CALL CSCAL(N-J, -A(J,J), A(J+1,J), 1)
               END IF
            END IF
            ! Regardless of what we have done above, we want to replace
            ! the current column of A with the solution to
