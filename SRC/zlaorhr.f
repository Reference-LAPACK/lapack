...
*
*> \htmlonly
*> Download ZLAORHR + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zlaorhr.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zlaorhr.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zlaorhr.f">
*> [TXT]</a>
*>
*  Definition:
*  ===========
*
*       SUBROUTINE ZLAORHR( M, N, MB1, NB1, A, LDA, T1, LDT1, NB2, T2,
...
*> \verbatim
*>
*> ZLAORHR is an auxiliary routine of ZORHR. ZLAORHR is a low-level
*> routine that does the computation. ZLAORHR performs Householder
...
*> QR factorization routine ZLATSQR. The reconstructed orthogonal M-by-M
...
*> output in A and T from QR factorization routine ZGEQRT. ZLAORHR uses
...
*> \verbatim
*>          MB1 is INTEGER
*>          The row block size used by ZLATSQR to compute the input
*>          arrays A and T1 (MB1 is called MB in ZLATSQR,
...
*> \verbatim
*>          NB1 is INTEGER
*>          The column block size used by ZLATSQR to compute the input
*>          arrays A and T1 (NB1 is called NB in ZLATSQR,
...
*>             computed by ZLATSQR. See Further Details.
*>
...
*>             of blocked matrix V computed by ZLATSQR
*>             (same format as the output A in ZLATSQR).
*>             See Further Details.
*>
...
*>             reflectors (same format as the output A in ZGEQRT).
*>             See Further Details.
*> \endverbatim
*>
*> \param[in] LDA
...
*> \param[in] T1
*> \verbatim
*>          T1 is COMPLEX*16 array, dimension (LDT1, N * NIRB),
*>
...
*> \param[out] T2
*> \verbatim
*>          T2 is COMPLEX*16 array, dimension (LDT2, N)
*>
...
*> \param[out] WORK
*> \verbatim
*>          WORK is (workspace) COMPLEX*16 array,
...
*> =========
*>
*> See the documentation of ZLATSQR.
*> On entry in A, the output from the M-by-N matrix A from ZLATSQR is
...
*> by ZLATSQR. The elements on and above the diagonal represent the
*> upper-triangular N-by-N matrix R_in computed by ZLATSQR.
*>
...
*>                   = CEIL(N/NB1)
*>
*> On entry in T1, the output from the matrix T from ZLATSQR. The matrix
...
*> size MB respectively used in ZLATSQR.
*>
*> ========
*> On exit:
*> ========
...
*> as the output from the ZGEQRT routine stored in the matrices A and T.
*> See the documentation for ZGEQRT.
*>
...
*     and zeros elsewhere.
*
      CALL ZLASET( 'F', M, N, CZERO, CONE, W, LDW )
*
...
*
      CALL ZLAORHR_GETRFNP( N, N, W, LDW, D, IINFO )
*
*     (2-2) Solve for L21.
*
      CALL ZTRSM( 'R', 'U', 'N', 'N', M-N, N, CONE, W, LDW,
     $            W( N+1, 1 ), LDW )
*
...
*
      DO J = 1, N
         CALL ZCOPY( M-J, W( J+1, J ), 1, A( J+1, J ), 1 )
      END DO
*
...
*
         JBTEMP1 = JB - 1
         DO J = JB, JB+JNB-1
            CALL ZCOPY( J-JBTEMP1, W( JB, J ), 1, T2( 1, J ), 1 )
         END DO
*
...
*
      DO I = 1, N
         IF( D( I ).EQ.-CONE ) THEN
            CALL ZSCAL( NPLUSONE-I, -CONE, A( I, I ), LDA )
         END IF
      END DO
*
      WORK( 1 ) = DCMPLX( LW )
