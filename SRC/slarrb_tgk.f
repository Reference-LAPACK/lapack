*> \brief \b SLARRB_TGK bisection refinement of eigenvalues of an LDL^T representation (Golub-Kahan variant of SLARRB); also SLARRBTGK_STURM4 (four-lane Sturm count) and SLARRBTGK_STURM_SLOW (reference Sturm count).
*>
*> \ingroup bdsvdmr3
*>
*> \par Purpose:
...
*>
*> \verbatim
*> bisection refinement of eigenvalues of an LDL^T representation (Golub-Kahan variant of SLARRB); also SLARRBTGK_STURM4 (four-lane Sturm count) and SLARRBTGK_STURM_SLOW (reference Sturm count).
...
*> \endverbatim
*
      SUBROUTINE SLARRB_TGK( N, D, L, LD, LLD, IFIRST, ILAST, RTOL1,
...
*     Derived from the LAPACK 3.0 SLARRB (1999); TGK adaptation 2026
*
*     .. Scalar Arguments ..
...
*     .. Parameters ..
      ...
      REAL   ZERO, ONE, TWO, HALF, TEN
      PARAMETER          ( ZERO = 0.0E0, ONE = 1.0E0, TWO = 2.0E0,
     $                   HALF = 0.5E0, TEN = 10.0E0 )
*     ..
*     .. Local Scalars ..
      INTEGER            B, BAFTER, CNT, I, II, I1, I2, ITER, J, K,
     $                   KK, NB, NEXT, NEXP, NLEFT, NINT, NRIGHT,
     $                   OLNINT, P, PREV
      REAL   DPLUS, EPS, ERROR, FAC, GAP, LEFT, MID, RIGHT,
     $                   OVFL, S, TMP, WIDTH
...
*
      ...
      DO 87 B = 1, NB
         ...
         BAFTER = BNEXT( B )
   87 CONTINUE
*     The four-lane count SLARRBTGK_STURM4 is kept below but not used;
*     each midpoint is counted by the reference one-shift routine.
      DO 88 B = 1, NB
         CALL SLARRBTGK_STURM_SLOW( N, D, LLD, BMID( B ), BCNT( B ) )
   88 CONTINUE
*
      DO 100 B = 1, NB
         I = BI( B )
...
*
      ...
      SUBROUTINE SLARRBTGK_STURM4( N, D, LLD, NB, X, CNT )
*
...
*
      ...
      REAL   DP1, DP2, DP3, DP4, S1, S2, S3, S4
*
      C1 = 0
      S1 = -X( 1 )
      IF( NB.EQ.1 ) THEN
...
*
      IF( .NOT.( S1.GT.0.0E0 .OR. S1.LT.1.0E0 ) )
     $   CALL SLARRBTGK_STURM_SLOW( N, D, LLD, X( 1 ), CNT( 1 ) )
      IF( NB.GE.2 ) THEN
         IF( .NOT.( S2.GT.0.0E0 .OR. S2.LT.1.0E0 ) )
     $      CALL SLARRBTGK_STURM_SLOW( N, D, LLD, X( 2 ), CNT( 2 ) )
      END IF
      IF( NB.GE.3 ) THEN
         IF( .NOT.( S3.GT.0.0E0 .OR. S3.LT.1.0E0 ) )
     $      CALL SLARRBTGK_STURM_SLOW( N, D, LLD, X( 3 ), CNT( 3 ) )
      END IF
      IF( NB.GE.4 ) THEN
         IF( .NOT.( S4.GT.0.0E0 .OR. S4.LT.1.0E0 ) )
     $      CALL SLARRBTGK_STURM_SLOW( N, D, LLD, X( 4 ), CNT( 4 ) )
      END IF
      RETURN
      END

      SUBROUTINE SLARRBTGK_STURM_SLOW( N, D, LLD, X, CNT )
*
*     NaN-safe scalar recurrence, identical to SLARRB's original slow
*     path.  It is split out only so each lane of SLARRBTGK_STURM4 can use it.
*
      INTEGER            N, CNT
      REAL   D( * ), LLD( * ), X
      INTEGER            J
...
*
      ...
      DO 10 J = 1, N - 1
         DPLUS = D( J ) + S
         IF( DPLUS.LT.0.0E0 ) CNT = CNT + 1
         TMP = LLD( J ) / DPLUS
         IF( TMP.EQ.0.0E0 ) THEN
            S = LLD( J ) - X
         ELSE
            S = S*TMP - X
         END IF
