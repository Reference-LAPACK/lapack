...
*
*> Download DKTEIN + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dktein.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dktein.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dktein.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*       SUBROUTINE DKTEIN( N, E, M, W, IBLOCK, ISPLIT, Z, LDZ, WORK,
*                          IWORK, IFAIL, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> DKTEIN computes the eigenvectors of a real skew-symmetric tridiagonal
...
*
*     .. Parameters ..
      DOUBLE PRECISION   ZERO, ONE, TWO, FOUR, TEN, ODM3, ODM1
      PARAMETER          ( ZERO = 0.0D+0, ONE = 1.0D+0,
     $                   TWO = 2.0D+0, FOUR = 4.0D+0, TEN = 1.0D+1,
...
*     .. External Subroutines ..
      EXTERNAL           DAXPY, DCOPY, DLAGTF, DLAGTS, DLAGTFK,
     $                   DLAGTSK, DLARNV, DSCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               DO I = 1, BLKSIZ
                  WORK( INDRV4+I ) = ZERO
               END DO
               CALL DCOPY( BLKSIZ-1, E( B1 ), 1, WORK( INDRV2+2 ),
     $                     1 )
               CALL DSCAL( BLKSIZ-1, -ONE, WORK( INDRV2+2 ), 1 )
               CALL DCOPY( BLKSIZ-1, E( B1 ), 1, WORK( INDRV3+1 ),
     $                     1 )
*
*              Compute LU factors with partial pivoting  ( PT = LU )
*
               TOL = ZERO
               CALL DLAGTFK( BLKSIZ, WORK( INDRV4+1 ), -XJ,
...
*
               CALL DLAGTSK( -1, BLKSIZ, WORK( INDRV4+1 ),
     $                       WORK( INDRV2+2 ), WORK( INDRV3+1 ),
     $                       WORK( INDRV5+1 ), IWORK,
...
*
     ...
     $            GPIND = J
               IF( GPIND.NE.J ) THEN
                  DO 160 I = GPIND, J - 1
                     CTR = -DDOT( BLKSIZ, WORK( INDRV1+1 ), 1,
     $                            Z( B1, I ), 1 )
                     CALL DAXPY( BLKSIZ, CTR, Z( B1, I ), 1,
     $                           WORK( INDRV1+1 ), 1 )
  160             CONTINUE
               END IF
*
