*> \brief <b> SGESVDMR3 computes the singular value decomposition (SVD) for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download SGESVDMR3 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sgesvdmr3.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sgesvdmr3.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sgesvdmr3.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*     SUBROUTINE SGESVDMR3( JOBU, JOBVT, RANGE, M, N, A, LDA, VL, VU,
...
*> \verbatim
*>
*>  SGESVDMR3 computes the singular value decomposition (SVD) of a real
...
*>
*>  SGESVDMR3 uses an eigenvalue problem for obtaining the SVD, which
*>  allows for the computation of a subset of singular values and
*>  vectors. See SBDSVDMR3 for details.
*>
...
*> \param[out] IWORK
*> \verbatim
*>          IWORK is INTEGER array, dimension (24*MIN(M,N))
*>          If INFO = 0, the first NS elements of IWORK are zero. If INFO > 0,
*>          then IWORK contains the indices of the eigenvectors that failed
*>          to converge in SBDSVDMR3.
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*>           > 0:  SBDSVDMR3 returned a positive INFO, passed through
*>                 unchanged: MIN(M,N)+1 if the bidiagonal contained
*>                 a NaN or Inf, MIN(M,N)+2 if the SBDSDC fallback
*>                 in SBDSVDMR3 returned a non-finite result, and any
...
*     .. External Subroutines ..
      EXTERNAL           SBDSVDMR3, SGEBRD, SGELQF, SGEQRF,
     $                   SLACPY,
...
*
     ...
     $                     ILAENV( 1, 'SORMLQ', ' ', M, M, -1, -1 ) )
            ...
            END IF
         END IF
         MINWRK = MINWRK + 5*MINMN*MINMN + 23*MINMN
...
*
            ITGKZ = ITEMP
            ITEMP = ITGKZ + N*(N*2+1)
            CALL SBDSVDMR3( 'U', JOBZ, RNGTGK, N, WORK( ID ),
     $                      WORK( IE ),
     $                      VL, VU, ILTGK, IUTGK, NS, S, WORK( ITGKZ ),
     $                      N*2, WORK( ITEMP ), LWORK-ITEMP+1, IWORK,
     $                      24*MINMN, INFO )
*
...
*
            ITGKZ = ITEMP
            ITEMP = ITGKZ + M*(M*2+1)
            CALL SBDSVDMR3( 'U', JOBZ, RNGTGK, M, WORK( ID ),
     $                      WORK( IE ),
     $                      VL, VU, ILTGK, IUTGK, NS, S, WORK( ITGKZ ),
     $                      M*2, WORK( ITEMP ), LWORK-ITEMP+1, IWORK,
     $                      24*MINMN, INFO )
*
*           If needed, compute left singular vectors.
*
...
*
            ITGKZ = ITEMP
            ITEMP = ITGKZ + M*(M*2+1)
            CALL SBDSVDMR3( 'L', JOBZ, RNGTGK, M, WORK( ID ),
     $                      WORK( IE ),
     $                      VL, VU, ILTGK, IUTGK, NS, S, WORK( ITGKZ ),
     $                      M*2, WORK( ITEMP ), LWORK-ITEMP+1, IWORK,
     $                      24*MINMN, INFO )
