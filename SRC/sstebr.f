...
*  ===========
*
*       SUBROUTINE SSTEBR( N, D, E, WORK, LWORK, IWORK, LIWORK, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> SSTEBR computes all eigenvalues of a symmetric tridiagonal matrix
...
*>
*> Unlike SSTEDC with COMPZ = 'N', SSTEBR does not store full secular
*> eigenvector blocks across merge levels for later replay.  It is a
*> distinct routine and does not replace SSTEDC or SSTERF.
*>
*> \endverbatim
*
*  Arguments:
...
*
      IF( INFO.EQ.0 ) THEN
         SMLSIZ = ILAENV( 9, 'SSTEBR', ' ', 0, 0, 0, 0 )
         IF( N.LE.SMLSIZ ) THEN
            LWMIN = 1
            LIWMIN = 1
         ELSE
*
*           BLO/BHI: 2*N; merge scratch: 14*N (see SLAED7_BR).
*
            LWMIN = MAX( 1, 16*N )
            LIWMIN = MAX( 1, 7*N )
         END IF
...
*
     ...
     $                   E( START ), M-1, INFO )
*
            CALL SLAED0_BR( M, D( START ), E( START ), WORK, IWORK,
     $                      INFO )
            IF( INFO.NE.0 ) THEN
