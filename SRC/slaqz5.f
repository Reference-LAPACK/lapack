...
*  ===========
*
*       SUBROUTINE SLAQZ5( WANTS, WANTQ, WANTZ, N, ILO, IHI, A, LDA, B, LDB,
...
*> \verbatim
*>
*> SLAQZ5 is identical to SLAQZ0 except that it calls SLAQZ6 instead of SLAQZ3
*>
*> \endverbatim
*
*  Arguments:
...
*>
*  =====================================================================
      subroutine slaqz5(wantS,wantQ,wantZ,n,ilo,ihi,A,ldA,B,ldB,alphar,
...
*
      ...
      nmin = ilaenv( 12,'SLAQZ5',jbcmpz,n,ilo,ihi,lwork )

      nwr = ilaenv( 13,'SLAQZ5',jbcmpz,n,ilo,ihi,lwork )
      nwr = max( 2,nwr )
      nwr = min( ihi-ilo+1,( n-1 ) / 3,nwr )

      nibble = ilaenv( 14,'SLAQZ5',jbcmpz,n,ilo,ihi,lwork )
      
      nsr = ilaenv( 15,'SLAQZ5',jbcmpz,n,ilo,ihi,lwork )
      nsr = min( nsr,( n+6 ) / 9,ihi-ilo )
      nsr = max( 2,nsr-mod( nsr,2 ) )

      rcost = ilaenv( 17,'SLAQZ5',jbcmpz,n,ilo,ihi,lwork )
      itemp1 = int(nsr/sqrt( 1+2*nsr/(real(rcost)/100*n) ))
      itemp1 = ((itemp1-1)/4)*4+4
      nbr = nsr+itemp1

      if( n .lt. nmin ) then
         call shgeqz(wantS,wantQ,wantZ,n,ilo,ihi,A,ldA,B,ldB,alphar,
     $      alphai,beta,Q,ldQ,Z,ldZ,work,lwork,info)
         return
      end if
...
*     Workspace query to slaqz6
      nw = max(nwr,nmin)
      call slaqz6(ilschur,ilq,ilz,n,ilo,ihi,nw,A,ldA,B,ldB,Q,ldQ,Z,ldZ,
     $   n_undeflated,n_deflated,alphar,alphai,beta,work,nw,work,nw,
     $   work,-1,aed_info)
      itemp1 = int(work(1))
*     Workspace query to slaqz4
      call slaqz4(ilschur,ilq,ilz,n,ilo,ihi,nsr,nbr,alphar,alphai,beta,
...
*              to the top and deflate it
               ...
               do k2 = k,istart2+1,-1
                  ...
                  call srot(k2-2-istartm+1,B(istartm,k2),1,B(istartm,
     $               k2-1),1,c1,s1)
                  call srot(min(k2+1,istop)-istartm+1,A(istartm,k2),1,
     $               A(istartm,k2-1),1,c1,s1)
                  ...
                  if(k2.lt.istop) then
                     ...
                     call srot(istopm-k2+1,A(k2,k2),ldA,A(k2+1,k2),ldA,
     $                  c1,s1)
                     call srot(istopm-k2+1,B(k2,k2),ldB,B(k2+1,k2),ldB,
     $                  c1,s1)
                     if(ilq) then
               ...
               if(istart2.lt.istop)then
                  call slartg(A(istart2,istart2),A(istart2+1,istart2),
     $               c1,s1,temp)
                  A(istart2,istart2) = temp
                  A(istart2+1,istart2) = zero

                  call srot(istopm-(istart2+1)+1,A(istart2,istart2+1),
     $               ldA,A(istart2+1,istart2+1),ldA,c1,s1)
                  call srot(istopm-(istart2+1)+1,B(istart2,istart2+1),
...
*        Time for AED
*
         call slaqz6(ilschur,ilq,ilz,n,istart2,istop,nw,A,ldA,B,ldB,Q,
...
*
   80 call shgeqz(wantS,wantQ,wantZ,n,ilo,ihi,A,ldA,B,ldB,alphar,alphai,
