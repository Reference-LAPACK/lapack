...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SGEMV, SGGQRF, SLASCL, SORMQR, SORMRQ,
     $                   SSCAL, STRTRS, XERBLA
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
...
*     SLASCL is called with both endpoints at or above one so that
*     forming the factor raises no underflow.
*
      ANRM = SLANGE( 'M', N, M, A, LDA, RWORK )
      KA = 0
...
*
      IF( KA.NE.KD )
     $   CALL SSCAL( M, SCALE( ONE, KA-KD ), X, 1 )
      IF( KB.NE.KD )
     $   CALL SSCAL( P, SCALE( ONE, KB-KD ), Y, 1 )
*
