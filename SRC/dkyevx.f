*> \brief <b> DKYEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors for KY matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download DKYEVX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dkyevx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dkyevx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dkyevx.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*       SUBROUTINE DKYEVX( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU, IL, IU,
...
*> \verbatim
*>
*> DKYEVX computes selected eigenvalues and, optionally, eigenvectors
...
*>          where NB is the max of the blocksize for DKYTRD and DORMTR
*>          returned by ILAENV.
*>
...
*
*  =====================================================================
      SUBROUTINE DKYEVX( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU, IL,
     $                   IU,
     $                   ABSTOL, M, W, Z, LDZ, WORK, LWORK, IWORK,
     $                   IFAIL, INFO )
      IMPLICIT NONE
...
*     .. External Subroutines ..
     ...
     $                   DKTEBZ,
     $                   DKTEIN, DKTEQR, DSWAP, DKYTRD, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      CALL DKYTRD( UPLO, N, A, LDA, WORK( INDE ), WORK( INDTAU ),
     $             WORK( INDWRK ), LLWORK, IINFO )
      IF(.NOT.LOWER)
     $   CALL DSCAL(N-1, -ONE, WORK( INDE ), 1)
*
*     If all eigenvalues are desired and ABSTOL is less than or equal to
*     zero, then call DORGTR and DKTEQR.  If this fails for some eigenvalue,
*     then try DKTEBZ.
*
      TEST = .FALSE.
      IF( INDEIG ) THEN
...
*      IF( ( ALLEIG .OR. TEST ) .AND. ( ABSTOL.LE.ZERO ) ) THEN
     ...
     $                   WORK( INDWRK ), LLWORK, IINFO )
         ...
         CALL DKTEQR( JOBZ, N, WORK( INDEE ), Z, LDZ,
     $                WORK( INDWRK ), INFO )
         IF( INFO.EQ.0 ) THEN
            DO 30 I = 1, N
               IFAIL( I ) = 0
...
*
      ...
      INDISP = INDIBL + N
      INDIWO = INDISP + N
      CALL DKTEBZ( RANGE, ORDER, N, VLL, VUU, IL, IU, ABSTLL,
     $             WORK( INDE ), M, NSPLIT, W, IWORK( INDIBL ),
     $             IWORK( INDISP ), WORK( INDWRK ),
     $             IWORK( INDIWO ), INFO )
*
      IF( WANTZ ) THEN
         CALL DKTEIN( N, WORK( INDE ), M, W, IWORK( INDIBL ),
     $                IWORK( INDISP ), Z, LDZ, WORK( INDWRK ), 
     $                IWORK( INDIWO ), IFAIL, INFO )
*
*        Apply orthogonal matrix used in reduction to tridiagonal
*        form to eigenvectors returned by DKTEIN.
*
         INDWKN = INDE
