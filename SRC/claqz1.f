...
*
         ...
         B( K + 1, K + 1 ) = TEMP
         B( K + 1, K ) = CZERO
         CALL CROT( K + 2 - ISTARTM + 1, A( ISTARTM, K + 1 ), 1,
     $       A( ISTARTM, K ), 1, C, S )
...
*
         ...
         A( K + 1, K ) = TEMP
         A( K + 2, K ) = CZERO
         CALL CROT( ISTOPM - K, A( K + 1, K + 1 ), LDA, A( K + 2,
     $       K + 1 ), LDA, C, S )
         CALL CROT( ISTOPM - K, B( K + 1, K + 1 ), LDB, B( K + 2,
     $       K + 1 ), LDB, C, S )
         IF ( ILQ ) THEN
            CALL CROT( NQ, Q( 1, K + 1 - QSTART + 1 ), 1, Q( 1,
     $          K + 2 - QSTART + 1 ), 1, C, CONJG( S ) )
         END IF
*
      END IF
*
