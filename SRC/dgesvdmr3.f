*> \brief <b> DGESVDMR3 computes the singular value decomposition (SVD) for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download DGESVDMR3 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dgesvdmr3.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dgesvdmr3.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dgesvdmr3.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*     SUBROUTINE DGESVDMR3( JOBU, JOBVT, RANGE, M, N, A, LDA, VL, VU,
...
*> \verbatim
*>
*>  DGESVDMR3 computes the singular value decomposition (SVD) of a real
...
*>
*>  DGESVDMR3 uses an eigenvalue problem for obtaining the SVD, which
*>  allows for the computation of a subset of singular values and
*>  vectors. See DBDSVDMR3 for details.
*>
...
*     .. External Subroutines ..
      EXTERNAL           DBDSVDMR3, DGEBRD, DGELQF, DGEQRF,
     $                   DLACPY,
...
*           (Workspace: need 5*N*N+37*N + 2*N*(N+1))
*
            ITGKZ = ITEMP
            ITEMP = ITGKZ + N*(N*2+1)
            CALL DBDSVDMR3( 'U', JOBZ, RNGTGK, N, WORK( ID ),
     $                      WORK( IE ),
...
*
            ITGKZ = ITEMP
            ITEMP = ITGKZ + M*(M*2+1)
            CALL DBDSVDMR3( 'U', JOBZ, RNGTGK, M, WORK( ID ),
     $                      WORK( IE ),
     $                      VL, VU, ILTGK, IUTGK, NS, S, WORK( ITGKZ ),
...
*
            ITGKZ = ITEMP
            ITEMP = ITGKZ + M*(M*2+1)
            CALL DBDSVDMR3( 'L', JOBZ, RNGTGK, M, WORK( ID ),
     $                      WORK( IE ),
     $                      VL, VU, ILTGK, IUTGK, NS, S, WORK( ITGKZ ),
     $                      M*2, WORK( ITEMP ), LWORK-ITEMP+1, IWORK,
     $                      24*MINMN, INFO )
