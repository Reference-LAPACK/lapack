...
*> which is the first N columns of a product of orthogonal
...
*>             The elements below the diagonal represent matrix V
*>             computed by DLATSQR that defeines the input
*>             matrices Q_in(k).
...
*        This workspcare is used to store array C(LDC, N) and WORK(LWORK)
...
*           in DLAMTSQR call and query the optimal size of the work array
*           WORK(LWORK) in DLAMTSQR call.
*
            LDC = M
            LC = LDC*N
            CALL DLAMTSQR( 'L', 'N', M, N, N, MB, NBLOCAL,
     $                     A, LDA, T, LDT, WORK, LDC,
     $                     WORK(2), -1, IINFO )
            LW = INT( WORK( 2 ) )
*
            LWORKOPT = LC+LW
