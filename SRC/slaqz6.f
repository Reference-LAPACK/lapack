...
*
*> \htmlonly
*> Download SLAQZ6 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/slaqz6.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/slaqz6.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/slaqz6.f">
*> [TXT]</a>
*> \endhtmlonly
*
*> \par Purpose:
...
*> \verbatim
*>
*> SLAQZ6 is identical to SLAQZ3 except that it calls SHGEQZ instead of SLAQZ0
*> \endverbatim
*
*  Arguments:
*  ==========
...
*>
*  =====================================================================
      subroutine slaqz6(ilschur,ilq,ilz,n,ilo,ihi,nw,A,ldA,B,ldB,Q,ldQ,
...
*     Local Scalars
      logical :: bulge
      integer :: jw,kwtop,kwbot,istopm,istartm,k,k2,stgexc_info,ifst,
...
*     Determine required workspace
      ifst = 1
      ilst = jw
      call stgexc(.true.,.true.,jw,A,ldA,B,ldB,Qc,ldQc,Zc,ldZc,ifst,
     $   ilst,work,-1,stgexc_info)
      itemp = int(work(1))
...
*     Transform window to real schur form
      ...
      call shgeqz('S','V','V',jw,1,jw,A(kwtop,kwtop),ldA,B(kwtop,kwtop),
...
*                 Not deflatable, move out of the way
                  ifst = kwbot-kwtop+1
                  ilst = k2
                  call stgexc(.true.,.true.,jw,A(kwtop,kwtop),ldA,
     $               B(kwtop,kwtop),ldB,Qc,ldQc,Zc,ldZc,ifst,ilst,work,
     $               lwork,stgexc_info)
                  if (stgexc_info .ne. 0) then
*                    write(*,*) "swap warning", stgexc_info
                  end if
                  k2 = k2+2
               end if
               k = k+2
...
*           2x2 eigenvalue block
            call slag2(A(k,k),ldA,B(k,k),ldB,safmin,beta(k),beta(k+1),
...
*        Reflect spike back, this will create optimally packed bulges
         ...
         call slartg(A(k,kwtop-1),A(k+1,kwtop-1),c1,s1,temp)
            ...
            A(k+1,kwtop-1) = zero
            k2 = max(kwtop,k-1)
         call srot(ihi-k2+1,A(k,k2),ldA,A(k+1,k2),ldA,c1,s1)
            call srot(ihi-(k-1)+1,B(k,k-1),ldB,B(k+1,k-1),ldB,c1,s1)
            call srot(jw,Qc(1,k-kwtop+1),1,Qc(1,k+1-kwtop+1),1,c1,s1)
         end do
...
*              Move double pole block down and remove it
               do k2 = k-1,kwbot-2
                  call slaqz2(.true.,.true.,k2,kwtop,kwtop+jw-1,kwbot,A,
...
*                 Move shift down
                  ...
                  B(k2+1,k2+1) = temp
                  B(k2+1,k2) = zero
                  call srot(k2+2-istartm+1,A(istartm,k2+1),1,A(istartm,
     $               k2),1,c1,s1)
                  call srot(k2-istartm+1,B(istartm,k2+1),1,B(istartm,
     $               k2),1,c1,s1)
                  call srot(jw,Zc(1,k2+1-kwtop+1),1,Zc(1,k2-kwtop+1),1,
     $               c1,s1)
            
                  call slartg(A(k2+1,k2),A(k2+2,k2),c1,s1,temp)
                  A(k2+1,k2) = temp
                  A(k2+2,k2) = zero
                  call srot(istopm-k2,A(k2+1,k2+1),ldA,A(k2+2,k2+1),ldA,
     $               c1,s1)
                  call srot(istopm-k2,B(k2+1,k2+1),ldB,B(k2+2,k2+1),ldB,
     $               c1,s1)
                  call srot(jw,Qc(1,k2+1-kwtop+1),1,Qc(1,k2+2-kwtop+1),
     $               1,c1,s1)
...
*              Remove the shift
               ...
               call srot(kwbot-istartm,B(istartm,kwbot),1,B(istartm,
     $            kwbot-1),1,c1,s1)
               call srot(kwbot-istartm+1,A(istartm,kwbot),1,A(istartm,
     $            kwbot-1),1,c1,s1)
               call srot(jw,Zc(1,kwbot-kwtop+1),1,Zc(1,kwbot-1-kwtop+1),
     $            1,c1,s1)
...
*     Apply Qc and Zc to rest of the matrix
      ...
      if (istopm-ihi > 0) then
         call sgemm('T','N',jw,istopm-ihi,jw,one,Qc,ldQc,A(kwtop,ihi+1),
     $      ldA,zero,work,jw)
      call slacpy('ALL',jw,istopm-ihi,work,jw,A(kwtop,ihi+1),ldA)
         call sgemm('T','N',jw,istopm-ihi,jw,one,Qc,ldQc,B(kwtop,ihi+1),
     $      ldB,zero,work,jw)
      call slacpy('ALL',jw,istopm-ihi,work,jw,B(kwtop,ihi+1),ldB)
      end if
      if (ilq) then
         call sgemm('N','N',n,jw,jw,one,Q(1,kwtop),ldQ,Qc,ldQc,zero,
     $      work,n)
         call slacpy('ALL',n,jw,work,n,Q(1,kwtop),ldQ)
      end if

      if (kwtop-1-istartm+1 > 0) then
         call sgemm('N','N',kwtop-istartm,jw,jw,one,A(istartm,kwtop),
     $      ldA,Zc,ldZc,zero,work,kwtop-istartm)
        call slacpy('ALL',kwtop-istartm,jw,work,kwtop-istartm,A(istartm,
     $     kwtop),ldA)
         call sgemm('N','N',kwtop-istartm,jw,jw,one,B(istartm,kwtop),
     $      ldB,Zc,ldZc,zero,work,kwtop-istartm)
        call slacpy('ALL',kwtop-istartm,jw,work,kwtop-istartm,B(istartm,
     $     kwtop),ldB)
      end if
      if (ilz) then
         call sgemm('N','N',n,jw,jw,one,Z(1,kwtop),ldZ,Zc,ldZc,zero,
     $      work,n)
