...
*> \verbatim
*>
*> ZHETRRK performs one of the symmetric rank k operations
*>
...
*
      NX = ILAENV(3, 'ZHETRRK', UPLOA // UPLOC // TRANS // DIAG,
     $      K, -1, -1, -1)
*
      IF(K.LT.NX.OR.K.EQ.1) THEN
         CALL ZHETRRK_LVL2(UPLOA, UPLOC, TRANS, DIAG, K,
     $         ALPHA, A, LDA, BETA, C, LDC)
         RETURN
      END IF
*
...
*
            CALL ZHETRRK(UPLOA, UPLOC, TRANS, DIAG, L, ALPHA,
     $            A, LDA, BETA, C, LDC)
*
*           Compute C_{2,2}
*           C_{2,2} = \alpha*A_{2,2}**H*A_{2,2} + \beta C_{2,2}
*
            CALL ZHETRRK(UPLOA, UPLOC, TRANS, DIAG, K-L, ALPHA,
     $            A(L+1,L+1), LDA, BETA, C(L+1,L+1), LDC)
*
*           C_{2,2} = \alpha*A_{1,2}**H*A_{1,2} + C_{2,2}
*
            CALL ZHERK(UPLOC, TRANS, K-L, L, ALPHA, A(1,L+1), LDA,
     $            ONE, C(L+1,L+1), LDC)
            IF(UPPERC) THEN
*
...
*
     ...
     $               'No Transpose', DIAG, L, K-L, ZALPHA, A, LDA,
     $               A(1, L+1), LDA, ZBETA, C(1,L+1), LDC)
            ELSE
*
*              Compute C_{2,1} = \alpha*A_{1,2}**H*A_{1,1} + \beta C_{2,1} (TRMMOOP)
*
               CALL ZTRMMOOP('Right', UPLOA, 'No Transpose',
     $               'Conjugate', DIAG, K-L, L, ZALPHA, A, LDA,
     $               A(1, L+1), LDA, ZBETA, C(L+1,1), LDC)
            END IF
         ELSE
*
...
*
            CALL ZHERK(UPLOC, TRANS, L, K-L, ALPHA, A(L+1,1), LDA,
     $            ONE, C, LDC)
*
...
*
               CALL ZTRMMOOP('Right', UPLOA, 'No Transpose',
     $               'Conjugate', DIAG, L, K-L, ZALPHA, A(L+1,L+1), LDA,
     $                A(L+1,1), LDA, ZBETA, C(1,L+1), LDC)
            ELSE
*
*              Compute C_{2,1} = \alpha*A_{2,2}**H*A_{2,1} + \beta*C_{2,1}
*
               CALL ZTRMMOOP('Left', UPLOA, 'Conjugate',
     $               'No Transpose', DIAG, K-L, L, ZALPHA,
     $               A(L+1,L+1), LDA, A(L+1, 1), LDA, ZBETA,
     $               C(L+1,1), LDC)
            END IF
         END IF
      ELSE
...
*
            CALL ZHERK(UPLOC, TRANS, L, K-L, ALPHA, A(1,L+1), LDA,
     $            ONE, C, LDC)
*
...
*
     ...
     $               'No Transpose', DIAG, L, K-L, ZALPHA,
     $               A(L+1,L+1), LDA, A(1, L+1), LDA, ZBETA,
     $               C(1,L+1), LDC)
            ELSE
*
...
*
            CALL ZHETRRK(UPLOA, UPLOC, TRANS, DIAG, L, ALPHA, A, LDA,
     $            BETA, C, LDC)
*
*           Compute C_{2,2}
*           C_{2,2} = \alpha*A_{2,2}*A_{2,2}**H + \beta*C_{2,2}
...
*
            CALL ZHERK(UPLOC, TRANS, K-L, L, ALPHA, A(L+1,1), LDA,
     $            ONE, C(L+1,L+1), LDC)
            IF(UPPERC) THEN
*
...
*
               CALL ZTRMMOOP('Left', UPLOA, 'No Transpose',
     $               'Conjugate', DIAG, L, K-L, ZALPHA, A, LDA,
     $               A(L+1,1), LDA, ZBETA, C(1,L+1), LDC)
            ELSE
*
...
*
               CALL ZTRMMOOP('Right', UPLOA, 'Conjugate',
     $               'No Transpose', DIAG, K-L, L, ZALPHA, A, LDA,
     $               A(L+1,1), LDA, ZBETA, C(L+1,1), LDC)
            END IF
         END IF
      END IF
      END SUBROUTINE
